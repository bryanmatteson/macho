use macho_dyld::resolve::PointerInventory;
use macho_dyld::{ChainedImportLookup, FunctionStartsOutcome};
use macho_evidence::SelectedImageEvidence;
...
fn selected_image_session_exposes_leaf_states_without_a_graph_layer() {
    ...
    let image = macho_core::format::parse_macho_file(&bytes).unwrap();
