...
impl<'a> PointerResolver<'a> {
    ...
    fn read_u32(&self, va: Va) -> Result<u32> {
        ...
        let arr: [u8; 4] = bytes.try_into().map_err(|_| {
            crate::error::Error::format("read_bytes_at_va returned unexpected length (want 4)")
        })?;
    ...
    fn read_u64(&self, va: Va) -> Result<u64> {
        ...
        let arr: [u8; 8] = bytes.try_into().map_err(|_| {
            crate::error::Error::format("read_bytes_at_va returned unexpected length (want 8)")
        })?;
