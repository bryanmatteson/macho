...
fn cpp_leaf_operates_directly_on_core_models_without_the_facade() {
    ...
    let vtables_from_vec =
        macho_cpp::VtableIndex::build_from_source(&bytes).expect("borrowed vector parses");
    assert!(vtables_from_vec.vtables().is_empty());

    let raw: &[u8] = &bytes;
    let typeinfo_from_slice =
        macho_cpp::build_typeinfo_index_from_source(raw).expect("borrowed slice parses");
...
fn cpp_source_requires_explicit_fat_architecture_selection() {
    ...
    let vtable_error = macho_cpp::VtableIndex::build_from_source(&bytes)
        .expect_err("fat source must require architecture selection");
    assert_eq!(vtable_error.kind, macho_cpp::CppErrorKind::Unsupported);

    let typeinfo_error = macho_cpp::build_typeinfo_index_from_source(&bytes)
        .expect_err("fat source must require architecture selection");
    assert_eq!(typeinfo_error.kind, macho_cpp::CppErrorKind::Unsupported);
}
