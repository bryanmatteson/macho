...
pub fn decode_swift_strict(
    ...
    let (class_vtable_entries, class_overrides) =
    ...
    let validated_conformances = match validate_conformance_list(macho, limits) {
    ...
    let index = match crate::SwiftTypeIndex::build_with_demangler(macho, &NoSymbolDemangler) {
        Ok(index) => index,
        Err(error) => {
            return rejected(
                total_entries,
    ...
    if decoded_conformances.len() as u64 != conformance_entries {
        return rejected(
            total_entries,
            "swift_decoder_record_lost",
            Some("__swift5_proto".into()),
    ...
    validated(SwiftDecodeBatchV1 {
        ...
        collector_outcomes: vec![
            ...
            collector(
                "conformances",
                if conformance_entries == 0 && conditional_requirement_entries == 0 {
                    SwiftCollectorStatusV1::Absent
                } else {
                    SwiftCollectorStatusV1::Complete
                },
            ...
            collector(
                "class_dispatch",
                if class_dispatch_entries == Some(0) {
                    SwiftCollectorStatusV1::Absent
                } else {
                    SwiftCollectorStatusV1::Complete
                },
