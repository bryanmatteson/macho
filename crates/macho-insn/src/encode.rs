...
mod tests {
    ...
    fn relocate_arm64_bl() {
        ...
        let original = crate::arm64::encode_branch_insn(0x4000, 0x4100, true).unwrap();
    ...
    fn arm64_branch_max_positive_range() {
        ...
        let insn = crate::decode_one(&bytes, 0x0, Arch::Arm64).unwrap();
        assert_eq!(crate::resolve_branch_target(&insn, 0x0), Some(max_target));
    }
    ...
    fn arm64_branch_max_negative_range() {
        ...
        assert_eq!(bytes.len(), 4);
        let insn = crate::decode_one(&bytes, from, Arch::Arm64).unwrap();
        assert_eq!(crate::resolve_branch_target(&insn, from), Some(0x0));
    }
    ...
    fn arm64_branch_max_range_round_trip() {
        ...
        let insn = crate::decode_one(&bytes, from, Arch::Arm64).unwrap();
        assert_eq!(crate::resolve_branch_target(&insn, from), Some(to));
    }
    ...
    fn relocate_arm64_bcond_preserves_target() {
        ...
        let insn = crate::decode_one(&relocated, 0x5200, Arch::Arm64).unwrap();
        assert_eq!(crate::resolve_branch_target(&insn, 0x5200), Some(0x5100));
    ...
    fn relocate_arm64_cbz_preserves_target() {
        ...
        let insn = crate::decode_one(&relocated, 0x1100, Arch::Arm64).unwrap();
        assert_eq!(crate::resolve_branch_target(&insn, 0x1100), Some(0x1040));
    }
    ...
    fn relocate_arm64_tbnz_preserves_target_and_bits() {
        ...
        let insn = crate::decode_one(&relocated, 0x2010, Arch::Arm64).unwrap();
        assert_eq!(crate::resolve_branch_target(&insn, 0x2010), Some(0x2020));
    ...
    fn relocate_arm64_adr_preserves_target() {
        ...
        let insn = crate::decode_one(&relocated, 0x1008, Arch::Arm64).unwrap();
        match insn.kind {
            crate::InsnKind::PcRelative(ref info) => {
    ...
    fn relocate_arm64_adrp_preserves_target_page() {
        ...
        let insn = crate::decode_one(&relocated, 0x4000, Arch::Arm64).unwrap();
        match insn.kind {
            crate::InsnKind::PcRelative(ref info) => {
    ...
    fn relocate_x86_64_jmp_preserves_target() {
        ...
        let relocated = relocate_insn(&bytes, 0x1000, 0x2000, Arch::X86_64).unwrap();
        let insn = crate::decode_one(&relocated, 0x2000, Arch::X86_64).unwrap();
        assert_eq!(crate::resolve_branch_target(&insn, 0x2000), Some(0x5000));
    }
