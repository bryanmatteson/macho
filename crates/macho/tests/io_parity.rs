...
fn assert_process_matches_injected(args: Vec<OsString>) -> macho::cli::CapturedRun {
    let process = run_process(&args);
    let injected = macho::cli::run_captured(args.clone());
...
fn production_and_injected_io_are_byte_identical() {
    ...
    let injected = macho::cli::run_captured(patch_args);
