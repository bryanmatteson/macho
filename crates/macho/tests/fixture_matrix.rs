use macho::core::{ParseErrorKind, ParseLimits, ParseMode, ParseOptions};
use macho_test_support::{CPU_TYPE_ARM64, CPU_TYPE_X86_64};

fn forensic_with(mut limits: ParseLimits, bytes: &[u8]) -> Result<(), macho::core::ParseError> {
    limits.max_sections = limits.max_sections.max(1);
    macho::core::parse_with_options(
        bytes,
        &ParseOptions {
...
fn parse_error(bytes: &[u8]) -> macho::core::ParseError {
    match macho::core::parse(bytes) {
...
fn thin_and_multi_arch_fat_fixtures_parse() {
    ...
    macho::core::parse(&arm).expect("valid thin fixture");
    let fat = macho_test_support::fat32(&[(CPU_TYPE_ARM64, 0, arm), (CPU_TYPE_X86_64, 0, x86)]);
    let parsed = macho::core::parse(&fat).expect("valid multi-arch fixture");
...
fn known_unknown_truncated_and_impossible_load_commands_are_distinct() {
    ...
    macho::core::parse(&known).expect("bounded known load commands");

    let unknown_bytes = macho_test_support::unknown_load_command_image();
    let unknown = macho::core::parse(&unknown_bytes).expect("bounded unknown command is preserved");
    assert_eq!(
        unknown
            .first_macho()
            .expect("thin image")
    ...
    for bytes in [
        ...
        assert!(matches!(
            error.context.last(),
            Some(macho::core::ContextFrame::LoadCommand { index: 0 })
        ));
    }
}
...
fn invalid_fileset_entries_are_rejected() {
    macho::core::parse(&macho_test_support::fileset64_arm64()).expect("valid fileset");
    for bytes in [
        macho_test_support::fileset64_truncated_command(),
        macho_test_support::fileset64_out_of_bounds(),
    ] {
        assert!(macho::core::parse(&bytes).is_err());
    }
}
