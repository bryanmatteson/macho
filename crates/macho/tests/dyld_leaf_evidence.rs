use macho::core::model::addr::Va;
use macho::core::model::container::{MachoContainer, SelectionKey};
use macho::core::model::header::ArchSpec;
use macho::metadata::dyld::resolve::{
    InventoryPointerTarget, PointerEncoding, PointerInventory, PointerResolver,
};
use macho::metadata::dyld::{
...
fn function_starts_are_strict_bounded_and_source_retaining() {
    ...
    assert!(
        decode_function_starts(
            &macho::core::format::parse_macho_file(&function_starts(&[1])).unwrap(),
            4
        )
        .is_err()
    );
    ...
    assert!(
        decode_function_starts(
            &macho::core::format::parse_macho_file(&overflowing).unwrap(),
            4
        )
        .is_err()
    );
    ...
    assert!(
        decode_function_starts(
            &macho::core::format::parse_macho_file(&trailing).unwrap(),
            4
        )
        .is_err()
    );
    ...
    let absent = macho::core::format::parse_macho_file(&absent_bytes).unwrap();
    assert_eq!(
    ...
    let duplicate = macho::core::format::parse_macho_file(&duplicate).unwrap();
...
fn exact_selected_fat_slice_controls_function_starts() {
    ...
    let container = macho::core::parse(&fat).unwrap();
...
fn weak_bind_over_rebase_retains_both_occurrences() {
    use macho::metadata::dyld::resolve::LegacyBindStream;
