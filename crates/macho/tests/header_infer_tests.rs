...
fn sample_bundle(language: HeaderLanguage) -> EvidenceBundle {
    EvidenceBundle {
        ...
        unresolved: vec![macho::extract::headers::UnresolvedGap {
            id: "gap.ret".into(),
...
impl HeaderInferenceModel for SequenceModel {
    fn infer(&self, _prompt: &PromptSet) -> macho::Result<String> {
        let mut guard = self.responses.lock().expect("lock");
        if guard.is_empty() {
            return Err(macho::Error::Validation("no more responses".into()));
        }
        Ok(guard.remove(0))
    }
}
...
fn cli_prompt_and_apply_flow() {
    ...
    let prompt_json: PromptSet = serde_json::from_slice(&prompt.stdout).expect("valid prompt json");
    ...
    let sidecar: macho::extract::headers::SidecarOutput =
...
fn cli_check_bundle_fails_for_invalid_bundle() {
    ...
    assert_eq!(report_json["valid"], false);
...
fn cli_validate_fails_for_invalid_model_output() {
    ...
    let report_json: serde_json::Value =
        serde_json::from_slice(&result.stdout).expect("valid validation json");
    assert_eq!(report_json["valid"], false);
