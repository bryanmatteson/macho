use macho::core::format::{ParseMode, ParseOptions};
use macho::core::{ContextFrame, ParseErrorKind};

#[test]
fn forensic_mode_retains_safe_validation_diagnostics() {
    let warning = macho_test_support::warning_bearing_image();
    let strict = macho::core::parse(&warning).expect("warnings do not reject strict parsing");
    assert!(strict.first_macho().is_some());
    let outcome = macho::core::parse_with_options(
        &warning,
        &ParseOptions {
    ...
    let strict_error = macho::core::parse(&recoverable_error).expect_err("strict validation fails");
    assert_eq!(strict_error.kind, ParseErrorKind::Validation);
    let forensic = macho::core::parse_with_options(
        &recoverable_error,
        &ParseOptions {
            mode: ParseMode::Forensic,
            ..ParseOptions::default()
...
fn hard_bounds_fail_in_both_modes_with_command_context() {
    ...
    for mode in [ParseMode::Strict, ParseMode::Forensic] {
        let error = match macho::core::parse_with_options(
            &bytes,
            &ParseOptions {
                mode,
                ..ParseOptions::default()
...
fn nested_fat_error_preserves_slice_command_and_span() {
    ...
    let error = macho::core::parse(&fat).expect_err("nested command is malformed");
    assert_eq!(error.code(), "parse.load_command.invalid");
...
fn unknown_load_command_is_bounded_and_preserved() {
    ...
    let container = macho::core::parse(&bytes).expect("unknown command is structurally valid");
    let macho = container.first_macho().expect("one image");
    assert!(matches!(
        macho.load_commands()[0].kind(),
        macho::core::model::load_command::LoadCommand::Unknown(command)
