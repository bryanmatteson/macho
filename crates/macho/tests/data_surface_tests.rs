    ...
    for vtable in &index.vtables {
        assert!(
            matches!(
                ...
                Some(macho::analysis::vtables::SlotTarget::OffsetToTop { .. })
            ),
...
fn regression_vtable_slots_resolve_function_names_on_chained_fixup_binaries() {
    ...
    let function_slot_count: usize = index
        ...
        .filter(|s| {
            matches!(
                &s.target,
                macho::analysis::vtables::SlotTarget::Function { .. }
            )
        })
        .count();
    ...
    let unknown_slot_count: usize = index
        ...
        .filter(|s| {
            matches!(
                &s.target,
                macho::analysis::vtables::SlotTarget::Unknown { .. }
            )
        })
        .count();
        ...
        if vtable.slots.len() >= 2 {
            assert!(
                matches!(
                    ...
                    macho::analysis::vtables::SlotTarget::TypeInfo { .. }
                ),
    ...
    for vtable in &index.vtables {
        ...
        assert!(
            matches!(
                ...
                macho::analysis::vtables::SlotTarget::OffsetToTop { .. }
            ),
    ...
    for vtable in &index.vtables {
        for slot in vtable.slots.iter().take(2) {
            assert!(
                !matches!(
                    &slot.target,
                    macho::analysis::vtables::SlotTarget::PureVirtual
                ),
