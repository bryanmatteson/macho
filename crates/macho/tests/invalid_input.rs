...
fn fat_binary_too_small_is_error() {
    ...
    let result = macho::core::format::fat::parse_fat_binary(&data);
...
fn fat_binary_duplicate_architectures_are_rejected() {
    ...
    let error = macho::core::format::fat::parse_fat_binary(&data).unwrap_err();
    assert_eq!(error.kind, macho::core::ParseErrorKind::InvalidFormat);
