    ...
    for macho in container.macho_files() {
        ...
        for imp in &graph.imports {
            ...
            match &imp.provider {
                ...
                _ => panic!("unsupported import provider for {}", imp.name),
            }
        }
    }
}
...
fn compat_with_provider() {
    ...
    let target_mach = target_container
        .first_macho()
        .expect("test container contains a Mach-O image");
    let provider_mach = provider_container
        .first_macho()
        .expect("test container contains a Mach-O image");
...
fn compat_fat_provider_matches_arch() {
    ...
    for macho in target_container.macho_files() {
        ...
        let prov_mach = provider_container.find_arch(cpu).unwrap_or_else(|| {
            provider_container
                .first_macho()
                .expect("test container contains a Mach-O image")
        });
