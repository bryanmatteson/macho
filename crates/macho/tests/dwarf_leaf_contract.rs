...
fn dwarf_leaf_operates_directly_on_core_models_without_the_facade() {
    ...
    assert!(!macho::metadata::dwarf::has_dwarf_sections(image));
    assert!(
        macho::metadata::dwarf::load_dwarf(image)
...
fn bounded_traversal_retains_units_dies_forms_and_physical_lines() {
    ...
    let container = macho::core::parse(&bytes).expect("DWARF fixture parses as Mach-O");
    let image = container.first_macho().expect("thin image");
    let traversal = macho::metadata::dwarf::traverse_dwarf(
        image,
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
...
fn malformed_unit_and_tight_budget_fail_closed() {
    ...
    let container = macho::core::parse(&bytes).expect("envelope remains valid");
    let error = macho::metadata::dwarf::traverse_dwarf(
    ...
    assert_eq!(
        error.kind,
        macho::metadata::dwarf::DwarfErrorKind::InvalidFormat
    );

    let bytes = dwarf4_macho_fixture();
    let container = macho::core::parse(&bytes).expect("fixture parses");
    let limits = macho::metadata::dwarf::DwarfTraversalLimits {
        max_entries: 1,
        ..macho::metadata::dwarf::DwarfTraversalLimits::default()
    };
    let error = macho::metadata::dwarf::traverse_dwarf(
    ...
    assert_eq!(
        error.kind,
        macho::metadata::dwarf::DwarfErrorKind::Unsupported
    );
}
...
fn legacy_range_lists_retain_raw_order_bases_and_exact_intervals() {
    ...
    let container = macho::core::parse(&bytes).expect("range fixture parses");
    let traversal = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
...
fn dwarf5_rnglistx_retains_index_and_closed_rle_records() {
    ...
    let container = macho::core::parse(&bytes).expect("DWARF5 fixture parses");
    let traversal = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
...
fn range_lists_reject_missing_terminators_unknown_opcodes_and_tight_budgets() {
    ...
    let container = macho::core::parse(&missing_terminator).expect("envelope parses");
    let error = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
    ...
    let container = macho::core::parse(&missing_dwarf5_terminator).expect("envelope parses");
    let error = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
    ...
    let container = macho::core::parse(&truncated_operand).expect("envelope parses");
    let error = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
    ...
    let container = macho::core::parse(&overflowing_uleb).expect("envelope parses");
    let error = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
    ...
    let container = macho::core::parse(&unresolved_address_index).expect("envelope parses");
    let error = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
    ...
    let container = macho::core::parse(&unknown_opcode).expect("envelope parses");
    let error = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
...
fn legacy_32_bit_address_width_resolves_without_64_bit_reads() {
    ...
    let container = macho::core::parse(&bytes).expect("32-bit-address DWARF envelope parses");
    let traversal = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
...
fn suppressed_raw_ranges_are_retained_as_partial_not_absent() {
    ...
    let container = macho::core::parse(&bytes).expect("envelope parses");
    let traversal = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
...
fn dwarf5_indexed_strings_are_resolved_to_exact_attribute_bytes() {
    ...
    let container = macho::core::parse(&bytes).expect("object parses");
    let traversal = macho::metadata::dwarf::traverse_dwarf(
        container.first_macho().expect("thin image"),
        macho::metadata::dwarf::DwarfTraversalLimits::default(),
    )
