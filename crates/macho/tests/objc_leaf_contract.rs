//! Leaf contract: `macho::metadata::objc` works without the `macho` façade.
...
fn objc_leaf_operates_directly_on_core_models_without_the_facade() {
    ...
    let metadata =
        macho::metadata::objc::parse_objc_metadata(image).expect("absence is not failure");
    assert!(metadata.classes.is_empty());
    assert!(metadata.categories.is_empty());
    assert!(metadata.protocols.is_empty());

    let scan = macho::metadata::objc::scan_objc_metadata(image).expect("scan without lists");
    assert!(scan.observations.is_empty());

    let metadata_from_vec = macho::metadata::objc::parse_objc_metadata_from_source(&bytes)
        .expect("borrowed vector parses");
    assert!(metadata_from_vec.classes.is_empty());

    let raw: &[u8] = &bytes;
    let scan_from_slice =
        macho::metadata::objc::scan_objc_metadata_from_source(raw).expect("borrowed slice parses");
    assert!(scan_from_slice.observations.is_empty());

    let imp_count = macho::metadata::objc::fold_method_imps_from_source(raw, 0usize, |count, _| {
        *count += 1;
        Ok(())
    })
...
fn objc_source_requires_explicit_fat_architecture_selection() {
    ...
    let error = macho::metadata::objc::parse_objc_metadata_from_source(&bytes)
        .expect_err("fat source must require architecture selection");
    assert_eq!(
        error.kind,
        macho::metadata::objc::ObjcErrorKind::Unsupported
    );
}
