...
fn strict_itanium_leaf_conserves_exact_single_inheritance_records() {
    ...
    assert!(serde_json::from_value::<macho::metadata::cpp::StrictRttiBatch>(hostile).is_err());
    ...
    forged["records"][1]["record"]["runtime_vtable"]["observation_ordinal"] =
        serde_json::json!(999);
    assert!(serde_json::from_value::<macho::metadata::cpp::StrictRttiBatch>(forged).is_err());
}
