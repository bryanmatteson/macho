...
use crate::mutate::format::constants::{FAT_MAGIC, FAT_MAGIC_64};
use crate::mutate::format::io::{Endian, pod};
use crate::mutate::format::parse_macho_file;
use crate::mutate::model::addr::{AddressMap, Rva, ThinFileOffset, Va};
use crate::mutate::model::header::{Bitness, FatMagic, MachoHeader};
use crate::mutate::model::load_command::ParsedLoadCommand;
...
pub struct OwnedFatArch {
    ...
    pub spec: crate::mutate::model::header::ArchSpec,
...
impl OwnedFatBinary {
    ...
    pub fn from_fat(
        fat: &crate::mutate::model::container::FatBinary<'_>,
        full_data: &[u8],
    ) -> Self {
        let arches = fat
            .arches()
...
impl MachoOwnedExt for crate::core::model::macho_file::MachoFile<'_> {
    fn to_owned_mach(&self) -> OwnedMachoFile {
