...
mod tests {
    use super::*;
    use crate::mutate::{AddSection, MutationErrorKind, MutationOperation};
    ...
    fn add_file_backed_section_commits_payload_and_metadata() {
        ...
        let reparsed = crate::core::parse(&committed).expect("candidate reparses");
    ...
    fn add_file_backed_section_rejects_unowned_trailing_bytes() {
        ...
        input.push(0xAA);
        let container = crate::core::parse(&input).expect("fixture with trailing data parses");
        let mut transaction = PatchTransaction::new(container.first_macho().unwrap());
        transaction
    ...
    fn add_section_encodes_32_bit_little_and_big_endian() {
        for big_endian in [false, true] {
            ...
            let container = crate::core::parse(&input).expect("32-bit fixture parses");
            let mut transaction = PatchTransaction::new(container.first_macho().unwrap());
            transaction.add_section(
