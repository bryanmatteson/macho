...
mod tests {
    ...
    fn instruction_failures_retain_typed_sources_and_location() {
        ...
        assert!(matches!(
            decode.source,
            Some(crate::mutate::patch::PatchErrorSource::Decode(_))
        ));
    ...
    fn plan_function_entry_patch_x86_64_pads_with_nops() {
        ...
        let nop_insn =
            crate::insn::decode_one(&plan.patch_bytes[5..], 0, crate::insn::Arch::X86_64).unwrap();
        assert_eq!(nop_insn.kind, crate::insn::InsnKind::Nop);
    ...
    fn nop_fill_x86() {
        ...
        // crate::insn uses Intel-recommended multi-byte NOP sequences.
        let nops = p.read_bytes(0x100, 5).unwrap();
        assert_eq!(nops.len(), 5);
        // Verify each byte decodes as NOP via crate::insn.
        let insn = crate::insn::decode_one(nops, 0, crate::insn::Arch::X86_64).unwrap();
        assert_eq!(insn.kind, crate::insn::InsnKind::Nop);
    }
