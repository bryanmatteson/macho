...
use crate::core::model::{MachoContainer, SelectionKey};

use crate::mutate::owned::OwnedFatBinary;
use crate::mutate::{MutationError, Result};
...
pub fn transform_container(
    ...
    mut transform: impl FnMut(&crate::core::MachoFile<'_>) -> Result<Vec<u8>>,
) -> Result<ContainerEditResult> {
    let parsed = crate::core::parse(bytes).map_err(MutationError::from)?;
    ...
    crate::core::parse(&output).map_err(MutationError::from)?;
...
fn image_architecture(image: &crate::core::MachoFile<'_>) -> String {
    crate::core::model::ArchSpec {
