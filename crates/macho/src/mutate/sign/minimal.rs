...
fn sign_container(
    ...
    let parsed = crate::core::parse(bytes).map_err(failed)?;
    match &parsed {
        ...
        MachoContainer::Fat(fat) => {
            ...
            let mut output = crate::mutate::owned::OwnedFatBinary::from_fat(fat, bytes);
...
fn signing_metadata(
    ...
    let macho = crate::core::format::parse_macho_file(bytes).map_err(failed)?;
    let prior = macho
        .ext::<crate::metadata::codesign::CodeSignature<'_>>()
        .ok();
...
fn finalize_signature(
    ...
    for _ in 0..8 {
        ...
        crate::core::format::parse_macho_file(&prepared).map_err(failed)?;
...
fn strip_existing_signature(bytes: &[u8]) -> Result<Vec<u8>, SignatureProviderError> {
    ...
    let mut transaction = crate::mutate::PatchTransaction::new(&macho);
    ...
    crate::core::format::parse_macho_file(&output).map_err(failed)?;
    Ok(output)
}
...
fn resize_terminal_segment(
    ...
    let macho = crate::core::format::parse_macho_file(&snapshot).map_err(failed)?;
...
pub(super) fn verify_signed_binary(
    ...
    let parsed = crate::core::parse(bytes).map_err(verification)?;
    match &parsed {
...
fn verify_code_slots(
    bytes: &[u8],
) -> Result<crate::metadata::codesign::CodeSignature<'_>, SignatureProviderError> {
    let macho = crate::core::format::parse_macho_file(bytes).map_err(verification)?;
    let signature = macho
        .ext::<crate::metadata::codesign::CodeSignature<'_>>()
    ...
    let code_blob = signature
        .blobs()
        .iter()
        .find(|blob| blob.blob_type == crate::metadata::codesign::BlobType::CodeDirectory)
    ...
    if let Some(entitlements) = signature
        .blobs()
        .iter()
        .find(|blob| blob.blob_type == crate::metadata::codesign::BlobType::Entitlements)
    {
