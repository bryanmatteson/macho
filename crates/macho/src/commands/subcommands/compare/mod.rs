...
use crate::analysis::diff::{ChangeSeverity, DiffDomain, diff_containers};
...
pub struct CompareArgs {
    ...
    new: PathBuf,
    /// Comparison mode
    #[arg(long, value_enum, default_value_t = CompareMode::Binary)]
    mode: CompareMode,
...
pub fn run(args: CompareArgs) -> Result<()> {
    ...
    if let Some(ref threshold) = args.fail_on {
        ...
        if report.findings.iter().any(|f| f.severity >= min_severity) {
            std::io::stdout().flush()?;
            anyhow::bail!("compare findings reached fail threshold {threshold}");
        }
    }
