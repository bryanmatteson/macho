...
use super::common::for_each_selected_mach;
...
pub fn run(args: DwarfArgs) -> Result<()> {
    if let Some(output_dir) = args.output_dir {
        run_extract(&args.path, args.arch.as_deref(), args.section.as_deref(), &output_dir)
    } else {
        run_view(&args.path, args.arch.as_deref(), args.json)
    }
}

fn run_view(path: &PathBuf, arch: Option<&str>, json: bool) -> Result<()> {
    let file =
