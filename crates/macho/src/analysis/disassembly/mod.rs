...
use crate::core::format::constants::{CPU_SUBTYPE_ARM64E, CPU_TYPE_ARM64, CPU_TYPE_X86_64};
use crate::core::model::addr::Va;
use crate::core::model::container::MachoContainer;
use crate::core::model::header::ArchSpec;
use crate::insn::Arch;
...
use crate::analysis::report::{
    Architecture, ArchitectureSelection, ContainerKind, ReportContainerIdentity,
};
...
pub fn resolve_architecture_selector(
    ...
    let matches: Vec<_> = available
        ...
        .filter(|candidate| {
            ArchSpec {
                cpu_type: crate::core::model::header::CpuType(candidate.cpu_type),
                cpu_subtype: crate::core::model::header::CpuSubtype(candidate.cpu_subtype),
            }
...
pub fn resolve_slice_selection(
    ...
    match matches.as_slice() {
        ...
        _ => Ok(SliceSelection::Many {
            architectures: crate::analysis::report::NonEmpty::new(matches)
...
fn streaming_inner(
    ...
    let container_hash = crate::analysis::report::ContentHash::new(
        crate::analysis::report::sha256_hex(container.bytes()),
    )
...
fn owned_report_bytes(report: &DisassemblyReport) -> u64 {
    ...
    use crate::analysis::report::disassembly::{DisassemblyRecord, ReportSelection};

    let mut bytes = report.container.content_sha256.capacity() as u64;
    bytes += (report.slices.capacity()
        * size_of::<crate::analysis::report::disassembly::DisassemblySlice>()) as u64;
    match &report.request.selection {
        ReportSelection::Sections { selectors } => {
            bytes += (selectors.capacity()
                * size_of::<crate::analysis::report::disassembly::ReportSectionSelector>())
                as u64;
            bytes += selectors
                .iter()
    ...
    for slice in &report.slices {
        ...
        bytes += slice
            ...
            .uuid
            .as_ref()
            .map_or(0, crate::analysis::report::CanonicalUuid::capacity) as u64;
        bytes += (slice.regions.capacity()
            * size_of::<crate::analysis::report::disassembly::DisassemblyRegion>())
            as u64;
        bytes += (slice.issues.capacity()
            * size_of::<crate::analysis::report::disassembly::DisassemblyIssue>())
            as u64;
        ...
        for region in &slice.regions {
            ...
            bytes += (region.labels.capacity()
                * size_of::<crate::analysis::report::disassembly::DisassemblyLabel>())
                as u64;
...
fn architecture_for_macho(macho: &crate::core::model::macho_file::MachoFile<'_>) -> Architecture {
    Architecture {
...
pub(crate) fn instruction_arch(architecture: Architecture) -> Result<Arch, DisassemblyError> {
    match architecture.cpu_type {
        ...
        CPU_TYPE_ARM64
            if architecture.cpu_subtype & crate::core::format::constants::CPU_SUBTYPE_MASK
                == CPU_SUBTYPE_ARM64E =>
        {
            Ok(Arch::Arm64e)
        }
