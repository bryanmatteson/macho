...
fn address_count_and_structured_target_are_exact() {
    ...
    assert!(matches!(
        ...
        DisassemblyRecord::Instruction {
            kind: crate::analysis::report::disassembly::InstructionKind::Branch,
...
fn exact_unknown_and_mixed_fat_unsupported_slices_fail_closed() {
    ...
    let container = crate::core::parse(&mixed).unwrap();
    assert_eq!(
...
fn clipped_instructions_and_arm_alignment_are_uniform() {
    ...
    let x86_container = crate::core::parse(&x86).unwrap();
    ...
    let arm_container = crate::core::parse(&arm).unwrap();
    ...
    for selection in [
        ...
        let container = crate::core::parse(&unaligned).unwrap();
        assert_eq!(
    ...
    let container = crate::core::parse(&unaligned_symbol).unwrap();
    assert_eq!(
...
fn exact_symbols_reject_missing_ambiguous_and_non_code_inputs() {
    ...
    let container = crate::core::parse(&ambiguous).unwrap();
    ...
    let container = crate::core::parse(&data).unwrap();
    assert_eq!(
...
fn malformed_nlist_and_export_metadata_are_fatal_only_for_symbol_selection() {
    for (bytes, lenient) in [
        ...
        let container = if lenient {
            crate::core::parse_with_options(
                &bytes,
                &crate::core::ParseOptions {
                    mode: crate::core::ParseMode::Forensic,
                    limits: crate::core::ParseLimits::default(),
                },
            )
            .unwrap()
            .container
        } else {
            crate::core::parse(&bytes).unwrap()
        };
        let error = disassemble(&container, &request(symbols(&["_main"]))).unwrap_err();
        assert_eq!(error.code(), SYMBOL_METADATA_INVALID_CODE);
...
fn thin_and_fat_identity_offsets_use_both_coordinate_systems() {
    ...
    let thin_container = crate::core::parse(&thin_bytes).unwrap();
    ...
    let fat_container = crate::core::parse(&fat_bytes).unwrap();
...
fn report_validator_rejects_inconsistent_wire_evidence() {
    ...
    let rejects = |mutate: fn(&mut serde_json::Value)| {
        ...
        mutate(&mut value);
        assert!(
            serde_json::from_value::<crate::analysis::report::disassembly::DisassemblyReport>(
                value
            )
            .is_err()
        );
    ...
    *gap_code = "analysis.disassembly.gap.unknown".to_owned();
    ...
    let objc_container = crate::core::parse(&objc).unwrap();
...
fn disassembly_work_bounds() {
    ...
    let alias_container = crate::core::parse(&alias_bytes).unwrap();
    ...
    let alias_two_n_container = crate::core::parse(&alias_two_n_bytes).unwrap();
    ...
    let objc_container = crate::core::parse(&objc_bytes).unwrap();
    let (_, objc_stats) =
    ...
    let region_container = crate::core::parse(&region_bytes).unwrap();
    ...
    let atomic_container = crate::core::parse(&atomic_bytes).unwrap();
    ...
    let malformed_container = crate::core::parse(&malformed).unwrap();
...
fn disassemble_survives_section_va_near_sign_boundary() {
    ...
    let container = crate::core::parse(&x86).unwrap();
    let report = disassemble(&container, &DisassemblyRequest::default()).unwrap();
    assert!(!report.slices[0].regions.is_empty());
    ...
    let container = crate::core::parse(&arm).unwrap();
    let report = disassemble(&container, &DisassemblyRequest::default()).unwrap();
    assert!(!report.slices[0].regions.is_empty());
}
...
fn streaming_output_retention_is_constant_in_instruction_count() {
    use crate::analysis::report::ReportContainerIdentity;
    use crate::analysis::report::disassembly::{
