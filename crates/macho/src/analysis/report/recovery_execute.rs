...
use crate::core::model::addr::types::ThinFileOffset;
use crate::core::model::container::MachoContainer;
use crate::core::{LoadCommand, MachoFile};
use crate::metadata::dwarf::types::CallingConvention as DwarfCallingConvention;
use crate::metadata::dwarf::{DwarfFunctionIndex, DwarfVariableIndex};
...
fn function_starts(macho: &MachoFile<'_>) -> crate::analysis::Result<Vec<u64>> {
    let Some(data) = macho
        .load_commands()
        .iter()
    ...
    while !reader.is_empty() {
        ...
        address = address.checked_add(delta).ok_or_else(|| {
            crate::analysis::AnalysisError::invalid("function-start address overflow")
        })?;
        result.push(address);
    }
    Ok(result)
...
fn enrich_itanium(slice: &mut SliceRecovery) {
    ...
    for entity in &mut slice.entities {
        ...
        let Some(record) = crate::analysis::reconstruct::cpp::symbol::parse_symbol(
            &raw,
        ...
        let qualifiers = FunctionQualifiers {
            ...
            reference: decl
                ...
                .as_ref()
                .map(|kind| match kind {
                    crate::analysis::reconstruct::cpp::CppRefQualifier::Lvalue => {
                        ReferenceKind::Lvalue
                    }
                    crate::analysis::reconstruct::cpp::CppRefQualifier::Rvalue => {
...
fn execute_rtti(macho: &MachoFile<'_>, slice: &mut SliceRecovery, limits: RecoveryLimits) {
    ...
    let target_set = targets
        ...
        .map(EntityId::as_str)
        .collect::<BTreeSet<_>>();
    let index = match crate::metadata::cpp::build_typeinfo_index(macho) {
        Ok(index) => index,
        Err(error) => {
            let diagnostic_id =
            ...
            payload: EvidencePayload::Rtti {
                value: RttiEvidence {
                    kind: match node.kind {
                        crate::metadata::cpp::CppTypeInfoKind::Class => RttiKind::ClassTypeInfo,
                        crate::metadata::cpp::CppTypeInfoKind::SingleInheritance => {
                            RttiKind::SiClassTypeInfo
                        }
                        crate::metadata::cpp::CppTypeInfoKind::VirtualMultipleInheritance => {
...
fn execute_vtables(macho: &MachoFile<'_>, slice: &mut SliceRecovery, limits: RecoveryLimits) {
    ...
    let target_set = targets
        ...
        .map(EntityId::as_str)
        .collect::<BTreeSet<_>>();
    let index = match crate::metadata::cpp::VtableIndex::build_limited(
        macho,
        limits.max_evidence_records as usize,
    ) {
        Ok(index) => index,
        Err(error) => {
            let diagnostic_id =
