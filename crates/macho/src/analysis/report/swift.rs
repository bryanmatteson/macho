...
use crate::core::{MachoFile, Section};
...
) -> crate::analysis::Result<SwiftReport> {
...
pub fn recover_swift_surface(macho: &MachoFile<'_>) -> crate::analysis::Result<SwiftReport> {
    ...
    let mut index = macho.ext::<crate::metadata::swift::SwiftTypeIndex>()?;
    if let Ok(metadata) = macho.ext::<crate::metadata::objc::ObjCMetadata>() {
        let runtime_types = metadata
            ...
            .map(|class| {
                (
                    class.name.clone(),
                    crate::metadata::swift::types::SwiftTypeKind::Class,
                )
            })
            .chain(
                metadata
                    ...
                    .map(|protocol| {
                        (
                            protocol.name.clone(),
                            crate::metadata::swift::types::SwiftTypeKind::Protocol,
                        )
                    }),
            );
        index.enrich_objc_runtime_types(
            runtime_types,
            &crate::metadata::swift::PureSwiftDemangler,
        )?;
    }
    ...
    for (ordinal, swift_type) in index.types.iter().enumerate() {
        ...
        let (entity_seed, identity_stability) = if unique {
            match swift_type.source {
                crate::metadata::swift::types::SwiftTypeSource::SwiftMetadata => (
        ...
        let descriptor = match (
            ...
            (
                ...
                value: SwiftDescriptorLocation {
                    ...
                    file_offset: macho
                        .address_map()
                        .va_to_thin_offset(crate::core::model::addr::Va(address))
                        .ok()
                        .map(|offset| offset.0),
                    section,
        ...
        let source = match swift_type.source {
            ...
            crate::metadata::swift::types::SwiftTypeSource::DemangledSymbol => {
                SwiftObservationSource::Nlist
            }
            crate::metadata::swift::types::SwiftTypeSource::ObjCMetadata => {
                SwiftObservationSource::TypeMetadata
            }
        ...
        let observation_location = swift_type.address.and_then(|virtual_address| {
            section_identity
                ...
                .map(|section| SwiftDescriptorLocation {
                    ...
                    file_offset: macho
                        .address_map()
                        .va_to_thin_offset(crate::core::model::addr::Va(virtual_address))
                        .ok()
                        .map(|offset| offset.0),
                    section,
                    relative_offset: None,
        ...
        let raw_evidence =
            if swift_type.source == crate::metadata::swift::types::SwiftTypeSource::SwiftMetadata {
                swift_type
                    .address
                    .and_then(|address| {
                        macho
                            .read_bytes_at_va(crate::core::model::addr::Va(address), 20)
                            .ok()
                    })
                        ...
                        type_name: field.type_name.clone().or_else(|| {
                            field.mangled_type.as_deref().and_then(|bytes| {
                                std::str::from_utf8(bytes)
                                    .ok()
                                    .and_then(crate::metadata::symbols::demangle::demangle_symbol)
                            })
                        }),
                        flags: field.flags,
                    })
...
fn swift_identity_key(value: &crate::metadata::swift::types::SwiftType) -> String {
    format!("{:?}|{:?}|{}", value.source, value.kind, value.name)
}

fn swift_state(value: &crate::metadata::swift::types::SwiftType) -> SwiftEntityState {
    match value.source {
        crate::metadata::swift::types::SwiftTypeSource::SwiftMetadata
            if value.name.starts_with("__C.") =>
        {
        ...
        crate::metadata::swift::types::SwiftTypeSource::ObjCMetadata => SwiftEntityState::Partial,
        _ => SwiftEntityState::Unknown,
    }
}

fn map_kind(value: crate::metadata::swift::types::SwiftTypeKind) -> SwiftTypeKind {
    match value {
        crate::metadata::swift::types::SwiftTypeKind::Class => SwiftTypeKind::Class,
        crate::metadata::swift::types::SwiftTypeKind::Struct => SwiftTypeKind::Struct,
        crate::metadata::swift::types::SwiftTypeKind::Enum => SwiftTypeKind::Enum,
        crate::metadata::swift::types::SwiftTypeKind::Protocol => SwiftTypeKind::Protocol,
        crate::metadata::swift::types::SwiftTypeKind::Unknown => SwiftTypeKind::Unknown,
