...
pub fn project_objc_headers(report: &mut ObjCReport) -> crate::analysis::Result<()> {
...
fn project_slice(slice: &mut ObjCSliceReport) -> crate::analysis::Result<()> {
    let selected = slice
        .selection
        .selected_entity_ids
        .iter()
    ...
    let source = syntax::render(&unit).map_err(|error| {
        crate::analysis::AnalysisError::validation(format!("render ObjC header: {error}"))
    })?;
    let reparsed = syntax::TreeSitterHeaderParser
        .parse(syntax::Language::ObjectiveC, &source)
        .map_err(|error| {
            crate::analysis::AnalysisError::validation(format!(
                "reparse rendered ObjC header: {error}\n{source}"
            ))
        })?;
    let validation =
        syntax::validate(&reparsed, syntax::ValidationLimits::default()).map_err(|error| {
            crate::analysis::AnalysisError::validation(format!("validate ObjC header: {error}"))
        })?;
    if !validation.syntax_valid || !validation.semantic_valid {
        return Err(crate::analysis::AnalysisError::validation(format!(
