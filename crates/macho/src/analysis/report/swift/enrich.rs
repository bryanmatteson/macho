...
pub(super) fn enrich_reflection(
    macho: &MachoFile<'_>,
    index: &crate::metadata::swift::SwiftTypeIndex,
    sections: &[&Section],
...
fn field_descriptor(macho: &MachoFile<'_>, owner: u64) -> Option<(u64, Vec<u8>)> {
    ...
    let relative_bytes: [u8; 4] = macho
        .read_bytes_at_va(crate::core::model::addr::Va(field), 4)
        .ok()?
        .try_into()
        .ok()?;
    ...
    let header = macho
        .read_bytes_at_va(crate::core::model::addr::Va(address), 16)
        .ok()?;
    ...
    let raw = macho
        .read_bytes_at_va(crate::core::model::addr::Va(address), length)
        .ok()?
        .to_vec();
    Some((address, raw))
}
...
fn demangle_type_reference(raw: &[u8]) -> Option<String> {
    ...
    crate::metadata::symbols::demangle::demangle_swift_symbol(raw)
        .or_else(|| crate::metadata::symbols::demangle::demangle_swift_symbol(&format!("$s{raw}")))
}
