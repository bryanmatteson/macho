...
fn parse_objc_members(
    ...
    (
        Vec<ObjectiveCMethod>,
        Vec<crate::analysis::header_syntax::ObjectiveCProperty>,
    ),
    ParseError,
> {
...
fn parse_objc_ivars(
    text: &str,
) -> Result<Vec<crate::analysis::header_syntax::ObjectiveCIvar>, ParseError> {
    let Some(open) = text.find('{') else {
        return Ok(Vec::new());
    };
    let close = matching_delimiter(text, open, '{', '}')
        .ok_or_else(|| ParseError::InvalidDeclaration(text.to_owned()))?;
    let mut access = crate::analysis::header_syntax::ObjectiveCAccess::Protected;
    ...
    for raw in split_top_level(&text[open + 1..close], ';') {
        ...
        for (directive, next) in [
            (
                "@public",
                crate::analysis::header_syntax::ObjectiveCAccess::Public,
            ),
            (
                "@protected",
                crate::analysis::header_syntax::ObjectiveCAccess::Protected,
            ),
            (
                "@private",
                crate::analysis::header_syntax::ObjectiveCAccess::Private,
            ),
            (
                "@package",
                crate::analysis::header_syntax::ObjectiveCAccess::Package,
            ),
        ] {
            if let Some(rest) = value.strip_prefix(directive) {
                access = next;
        ...
        ivars.push(crate::analysis::header_syntax::ObjectiveCIvar {
...
fn parse_objc_property(
    text: &str,
) -> Result<crate::analysis::header_syntax::ObjectiveCProperty, ParseError> {
    ...
    let (ty, name) = split_type_and_name(value)?;
    Ok(crate::analysis::header_syntax::ObjectiveCProperty {
        name: parse_identifier(name.trim_start_matches('*').trim())?,
        ty: parse_type(ty, Language::ObjectiveC)?,
        attributes,
    })
...
fn parse_objc_property_attribute(
    value: &str,
) -> Result<crate::analysis::header_syntax::ObjectiveCPropertyAttribute, ParseError> {
...
pub(super) fn parse_function_qualifiers(text: &str) -> FunctionQualifiers {
    FunctionQualifiers {
        ...
        reference: if text.contains("&&") {
            Some(crate::analysis::header_syntax::ReferenceKind::Rvalue)
        } else if text.contains('&') {
            Some(crate::analysis::header_syntax::ReferenceKind::Lvalue)
        } else {
            None
        },
