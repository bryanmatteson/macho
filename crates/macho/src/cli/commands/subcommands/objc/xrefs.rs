...
use crate::core::model::symbol::{SymbolTable, SymbolType};
...
pub(super) struct XrefView {
    ...
    pub(super) references: Vec<crate::analysis::xref::Xref>,
}

fn xref_views(
    slice: &ObjCSliceReport,
    macho: &crate::core::model::macho_file::MachoFile<'_>,
) -> Vec<XrefView> {
    let selected = slice
        .selection
        .selected_entity_ids
    ...
    let xrefs = crate::analysis::xref::XrefIndex::build(macho).ok();
        ...
        for method in entity_methods(entity) {
            ...
            result.push(XrefView {
                ...
                references: xrefs
                    ...
                    .map(|index| {
                        index
                            .refs_to(crate::core::model::addr::Va(implementation.virtual_address))
                            .cloned()
                            .collect()
                    })
                    .unwrap_or_default(),
