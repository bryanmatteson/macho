use crate::cli::analysis::container::ContainerDocumentReport;
use crate::cli::analysis::container::ext::MachoContainerExt;
use crate::cli::analysis::{AnalysisDomain, Analyzer, ContainerPlan};
...
use crate::cli::commands::{OutputFormat, input_message};
...
fn run_list(
    path: &std::path::Path,
    arch: Option<&str>,
    limits: crate::cli::analysis::AnalysisLimits,
    format: OutputFormat,
    out: &mut dyn Write,
) -> Result<()> {
    let mmap = map_input(path)?;
    let container =
        crate::parse(&mmap).with_context(|| format!("failed to parse {}", path.display()))?;
