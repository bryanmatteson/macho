use crate::cli::analysis::{AnalysisDomain, AnalysisPlan};
...
use crate::cli::mutate::AddSection;
use crate::cli::mutate::PatchOp;
use crate::cli::mutate::owned::OwnedFatBinary;
use crate::cli::mutate::preview::{SignatureOutcome, StructuralPatchPreview};
use crate::cli::mutate::transaction::{PatchPlan, PreparedPatch};
use crate::cli::mutate::{
    InProcessSignatureProvider, SignatureKind, SignatureProvider, SignatureRequest,
};
use crate::cli::workflow::WorkflowSigning;
...
use crate::cli::commands::subcommands::common::arch_name_for_mach;
...
fn run_patch(
    ...
    let container =
        crate::parse(&mmap).with_context(|| format!("failed to parse {}", input.display()))?;
    ...
    let (output_bytes, previews) = match &container {
        ...
        MachoContainer::Fat(fat) => {
            ...
            let reparsed = crate::parse(&rebuilt).with_context(|| {
...
fn prepare_patch(
    ...
    let result = crate::workflow::execute(macho.bytes(), &patch_plan, &analysis, workflow_signing)?;
    let prepared = PreparedPatch {
        preview: result.preview.structural,
        bytes: result.bytes,
    };
    let reparsed = crate::parse(&prepared.bytes).context("failed to reparse prepared slice")?;
...
fn select_fat_arch_indices(
    fat: &crate::cli::model::container::FatBinary<'_>,
    arch_filter: Option<&str>,
) -> Result<Vec<usize>> {
...
mod tests {
    use super::*;
    use crate::cli::mutate::preview::StructuralPatchPreview;
