use crate::cli::format::constants::VmProtection;
use crate::cli::model::container::MachoContainer;
use crate::cli::model::load_command::format_uuid;
use crate::cli::model::macho_file::MachoFile;
use crate::cli::model::validate;
use anyhow::{Context, Result};
use std::io::Write;
use std::path::PathBuf;

use crate::cli::commands::args::{ArchitectureArgs, InputArgs};
use crate::cli::commands::output::Style;
use crate::cli::commands::output::layout::{self, Cell};
use crate::cli::commands::subcommands::common::{arch_name_for_mach, map_input};
use crate::cli::commands::{CliWarning, input_message};
...
pub(crate) fn run_scoped(
    ...
    let options = crate::core::ParseOptions {
        mode: crate::core::ParseMode::Forensic,
        limits: crate::core::ParseLimits::default(),
    };
    let outcome = crate::parse_with_options(&mmap, &options)
...
fn format_segment_row(segment: &crate::cli::model::segment::Segment, style: Style) -> Vec<Cell> {
    vec![
        layout::join_cells([
            layout::plain_cell("  "),
...
fn format_nested_section_row(
    section: &crate::cli::model::section::Section,
    style: Style,
) -> Vec<Cell> {
...
fn format_section_row(
    segment: &crate::cli::model::segment::Segment,
    section: &crate::cli::model::section::Section,
    style: Style,
) -> Vec<Cell> {
    format_section_cells(
...
fn print_summary(macho: &MachoFile<'_>, style: Style, out: &mut dyn Write) {
    ...
    if let Ok(fixups) = crate::cli::metadata::dyld::parse_chained_fixups(macho) {
        let _ = writeln!(
            out,
            "{} {} imports, {} fixups",
    ...
    if let Ok(exports) = crate::cli::metadata::dyld::parse_exports(macho)
        && !exports.is_empty()
    {
        let _ = writeln!(
            out,
...
fn format_section_extras(sect: &crate::cli::model::section::Section, style: Style) -> Cell {
    use crate::cli::model::section::SectionType;
