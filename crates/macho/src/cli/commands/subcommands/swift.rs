    ...
    for slice in report.slices.as_slice() {
        ...
        for diagnostic in &slice.diagnostics {
            ...
            let _ = writeln!(
                out,
                "  {}  {}",
                style.warning(diagnostic_code_name(diagnostic.code)),
                diagnostic.message
            );
        }
    }
...
fn diagnostic_code_name(code: crate::analysis::report::SwiftDiagnosticCode) -> &'static str {
    use crate::analysis::report::SwiftDiagnosticCode;

    match code {
        SwiftDiagnosticCode::MalformedDescriptor => "malformed-descriptor",
        SwiftDiagnosticCode::UnsupportedDescriptor => "unsupported-descriptor",
        SwiftDiagnosticCode::MalformedMangling => "malformed-mangling",
        SwiftDiagnosticCode::UnsupportedMangling => "unsupported-mangling",
        SwiftDiagnosticCode::UnresolvedReference => "unresolved-reference",
        SwiftDiagnosticCode::AmbiguousIdentity => "ambiguous-identity",
        SwiftDiagnosticCode::ConflictingMetadata => "conflicting-metadata",
        SwiftDiagnosticCode::CollectorFailed => "collector-failed",
        SwiftDiagnosticCode::CollectorTruncated => "collector-truncated",
    }
}
...
fn architecture_name(architecture: crate::analysis::report::Architecture) -> String {
...
mod tests {
    use super::*;
    use crate::analysis::report::{HexBytes, NonEmpty, SwiftEvidenceId};
    ...
    #[test]
    fn diagnostic_codes_are_readable_words() {
        assert_eq!(
            diagnostic_code_name(crate::analysis::report::SwiftDiagnosticCode::UnresolvedReference),
