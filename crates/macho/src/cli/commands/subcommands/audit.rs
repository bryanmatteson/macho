use crate::cli::analysis::audit::{AuditFinding, AuditReport, AuditSeverity};
use crate::cli::analysis::{AnalysisDomain, Analyzer, AuditPlan, DomainPayload, DomainState};
use crate::cli::commands::args::{AnalysisLimitArgs, ArchitectureArgs, InputArgs};
use anyhow::{Context, Result};
use serde::Serialize;
use std::io::Write;

use crate::cli::commands::output::sarif;
...
fn reports_from_document(
    document: &crate::cli::analysis::SnapshotDocument,
...
fn print_json(reports: &[AuditReport], out: &mut dyn Write) -> Result<()> {
    let payload: Vec<AuditJsonSlice<'_>> = reports
        ...
        .collect();
    crate::cli::commands::output::json::write_pretty(out, &payload)?;
    Ok(())
}
