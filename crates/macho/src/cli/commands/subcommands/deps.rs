use crate::cli::analysis::deps::compat::{CompatReport, CompatSeverity};
use crate::cli::analysis::deps::graph::{DepGraph, ImportProvider};
use crate::cli::commands::args::{ArchitectureArgs, InputArgs};
use crate::cli::commands::input_message;
...
use crate::cli::commands::subcommands::common::{
    arch_name_for_mach, map_input, write_selected_json,
};
...
pub fn run(args: DepsArgs, output: OutputOptions, out: &mut dyn Write) -> Result<()> {
    let json = output.format() == crate::cli::commands::OutputFormat::Json;
    ...
    let provider = if let Some(ref prov_path) = args.check_compat {
        ...
        provider_container = crate::parse(&provider_mmap)
...
fn print_deps(
    ...
    if json {
        ...
        crate::cli::commands::output::json::write_pretty(out, &output)?;
...
mod tests {
    ...
    use crate::cli::analysis::deps::graph::{DepGraph, NormalizedDylib};
    use crate::cli::commands::output::Style;
    use crate::cli::metadata::image::DylibLinkKind;
