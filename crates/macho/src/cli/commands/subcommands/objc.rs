...
use crate::cli::commands::args::{ArchitectureArgs, InputArgs, OptionalInputArgs};
use crate::cli::commands::output::layout::{self, Cell};
use crate::cli::commands::output::{Options as OutputOptions, Style, objc};
...
fn entity_row(entity: &ObjCEntity, style: Style) -> Vec<Cell> {
    ...
    let name = known(&common.name)
        .map(|name| crate::symbols::demangle::demangle_swift_symbol(name).unwrap_or(name.clone()))
...
fn property_row(value: &crate::analysis::report::ObjCProperty, style: Style) -> Vec<Cell> {
    vec![
...
fn ivar_row(value: &crate::analysis::report::ObjCIvar, style: Style) -> Vec<Cell> {
    vec![
...
fn run_graph(
    ...
    if output.format() == OutputFormat::Json {
        return crate::cli::commands::output::json::write_pretty(out, &views).map_err(Into::into);
    }
    for view in views {
        writeln!(
            out,
...
struct GraphView<'a> {
    ...
    selector_owners: Vec<&'a crate::analysis::report::ObjCSelectorOwner>,
}
