...
use crate::metadata::objc::format::io::pod::{
    self, RawMethodListHeader, RawMethodT, RawRelativeMethodT,
};
use crate::metadata::objc::model::addr::{ThinFileOffset, Va};
...
pub fn parse_method_list(resolver: &ObjCResolver<'_>, va: Va) -> Result<Vec<ObjCMethod>> {
    ...
    if uses_relative {
        ...
        if entsize < size_of::<RawRelativeMethodT>() {
            return Err(crate::metadata::objc::error::Error::format(format!(
        ...
        for i in 0..count {
            let entry_offset = offset
                ...
                .ok_or_else(|| {
                    crate::metadata::objc::error::Error::format(format!(
                        "relative method[{i}] offset overflows"
                    ))
                })?;
            ...
            let types_field_addr = entry_offset.checked_add(4).ok_or_else(|| {
                crate::metadata::objc::error::Error::address("method type field overflows")
            })?;
            let imp_field_addr = entry_offset.checked_add(8).ok_or_else(|| {
                crate::metadata::objc::error::Error::address("method IMP field overflows")
            })?;
            ...
            let selector_offset =
                name_field_addr
                    ...
                    .ok_or_else(|| {
                        crate::metadata::objc::error::Error::address(
                            "method selector offset overflows",
                        )
                    })?;
            ...
            let types_file_offset =
                types_field_addr
                    ...
                    .ok_or_else(|| {
                        crate::metadata::objc::error::Error::address(
                            "method type-encoding offset overflows",
                        )
                    })?;
            ...
            let imp_addr = imp_field_va
                ...
                .ok_or_else(|| {
                    crate::metadata::objc::error::Error::address(
                        "relative method IMP address is invalid",
                    )
                })?;
        ...
        for i in 0..count {
            let entry_offset = offset
                ...
                .ok_or_else(|| {
                    crate::metadata::objc::error::Error::format(format!(
                        "absolute method[{i}] offset overflows"
                    ))
                })?;
            ...
            let name_ptr = resolver
                ...
                .ok_or_else(|| {
                    crate::metadata::objc::error::Error::address(
                        "method name pointer is unresolved",
                    )
                })?;
            let types_ptr = resolver
                ...
                .ok_or_else(|| {
                    crate::metadata::objc::error::Error::address(
                        "method type-encoding pointer is unresolved",
                    )
                })?;
            let imp = resolver
                ...
                .ok_or_else(|| {
                    crate::metadata::objc::error::Error::address("method IMP pointer is unresolved")
                })?;
...
fn read_cstring_at_file_offset(data: &[u8], offset: usize) -> Result<&str> {
    let slice = data.get(offset..).ok_or_else(|| {
        crate::metadata::objc::error::Error::bounds(offset as u64, 1, data.len() as u64)
    })?;
    let end = slice.iter().position(|&b| b == 0).ok_or_else(|| {
        crate::metadata::objc::error::Error::format(format!(
            "unterminated string at file offset {offset:#x}"
        ))
    })?;
    std::str::from_utf8(&slice[..end]).map_err(|e| {
        crate::metadata::objc::error::Error::format(format!(
            "invalid UTF-8 at offset {offset}: {e}"
        ))
    })
}
