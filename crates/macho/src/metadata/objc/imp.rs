...
use crate::metadata::objc::resolve::{ObjCPointerProvenance, ObjCResolver};
...
/// selection through [`crate::core::parse`] and [`fold_method_imps`].
...
where
    S: AsRef<[u8]> + ?Sized,
{
    let macho = crate::metadata::objc::parse_source(source)?;
...
fn fold_class<State>(
    ...
    let ro_offset = resolver
        .va_to_offset(Va(
            data_va.0 & crate::metadata::objc::types::CLASS_DATA_POINTER_MASK
        ))?
        .0;
    ...
    if let Some(meta_va) = resolver.read_pointer_at_offset(class_offset)? {
        ...
        let meta_ro_offset = resolver
            .va_to_offset(Va(
                meta_data.0 & crate::metadata::objc::types::CLASS_DATA_POINTER_MASK
            ))?
            .0;
...
mod tests {
    ...
    fn thin(bytes: &[u8]) -> &MachoFile<'_> {
        let container = Box::leak(Box::new(crate::core::parse(bytes).unwrap()));
        match container {
