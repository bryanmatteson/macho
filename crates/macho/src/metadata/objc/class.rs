...
use crate::metadata::objc::format::io::pod::{self, RawClassRoT64};
use crate::metadata::objc::ivar::parse_ivar_list;
use crate::metadata::objc::method::parse_method_list;
use crate::metadata::objc::model::addr::Va;
use crate::metadata::objc::property::{parse_property_list, parse_property_list_with_kind};
use crate::metadata::objc::protocol::parse_protocol_name_list;
use crate::metadata::objc::resolve::ObjCResolver;
use crate::metadata::objc::types::{ObjCClass, ObjCMethod, ObjCProperty, RO_META};

/// Performs parse_class.
pub fn parse_class(resolver: &ObjCResolver<'_>, class_va: Va) -> Result<ObjCClass> {
    ...
    let raw_data_ptr = match resolver.read_pointer_at_offset(data_ptr_offset)? {
        Some(va) => va,
        None => {
            return Err(crate::metadata::objc::error::Error::format(
                "null class_ro_t pointer",
            ));
        }
    };
    let is_swift = raw_data_ptr.0 & 1 != 0;
    let data_va = Va(raw_data_ptr.0 & crate::metadata::objc::types::CLASS_DATA_POINTER_MASK);
...
pub(crate) fn resolve_class_name_from_va(
    ...
    let data_va = Va(data_va.0 & crate::metadata::objc::types::CLASS_DATA_POINTER_MASK);
...
fn parse_metaclass_surface(
    ...
    let data_va = match resolver.read_pointer_at_offset(data_ptr_offset)? {
        Some(va) => Va(va.0 & crate::metadata::objc::types::CLASS_DATA_POINTER_MASK),
