...
use crate::metadata::objc::format::io::endian::Endian;
use crate::metadata::objc::format::io::pod;
use crate::metadata::objc::model::addr::{ThinFileOffset, Va};
use crate::metadata::objc::model::load_command::LoadCommand;
...
fn build_legacy_fixup_map(macho: &MachoFile<'_>) -> Result<HashMap<u64, ResolvedFixup>> {
    ...
    let (regular, weak, lazy) = crate::metadata::dyld::bind::parse_bind_entries(macho)?;
    ...
    for entry in crate::metadata::dyld::rebase::parse_rebase_entries(macho)? {
        let seg = macho.segments().get(entry.segment_index).ok_or_else(|| {
            Error::format(format!(
