...
use crate::metadata::objc::format::io::pod::{self, RawClassRoT64, RawProtocolT64};
use crate::metadata::objc::model::addr::{ThinFileOffset, Va};
use crate::metadata::objc::model::macho_file::MachoFile;
use crate::metadata::objc::resolve::ObjCResolver;
use crate::metadata::objc::{
...
pub fn class_metaclass_reference(
    ...
    class_metaclass_reference_with_resolver(macho, &resolver, class_va)
}

pub(crate) fn class_metaclass_reference_with_resolver(
    macho: &MachoFile<'_>,
...
fn validate_optional_method_list(resolver: &ObjCResolver<'_>, pointer_offset: u64) -> Result<()> {
    if let Some(value) = resolver.read_pointer_at_offset(pointer_offset)?
        && value.0 != 0
    {
        crate::metadata::objc::method::parse_method_list(resolver, value)?;
    }
    Ok(())
}
...
fn validate_optional_ivar_list(resolver: &ObjCResolver<'_>, pointer_offset: u64) -> Result<()> {
    if let Some(value) = resolver.read_pointer_at_offset(pointer_offset)?
        && value.0 != 0
    {
        crate::metadata::objc::ivar::parse_ivar_list(resolver, value)?;
    }
    Ok(())
}
...
fn validate_optional_property_list(
    ...
    if let Some(value) = resolver.read_pointer_at_offset(pointer_offset)?
        && value.0 != 0
    {
        crate::metadata::objc::property::parse_property_list_with_kind(resolver, value, is_class)?;
    }
    Ok(())
}
...
fn validate_optional_protocol_list(
    ...
    let names = crate::metadata::objc::protocol::parse_protocol_name_list(resolver, value)?;
    if names.len() != count {
        return Err(Error::format(format!(
...
const fn objc_class_ro_pointer(value: Va) -> Va {
    Va(value.0 & crate::metadata::objc::types::CLASS_DATA_POINTER_MASK)
}
