...
use crate::metadata::dwarf::{Error, MachoFile, Result};
...
pub fn traverse_dwarf(
    ...
    // Exception-frame sections use DWARF encodings but do not constitute a
    // source-debug package. They are recovered by the exception/CFI pipeline
    // and must not make an otherwise stripped image look like malformed DWARF.
    if !crate::metadata::dwarf::has_dwarf_sections(macho) {
        return Ok(None);
    }
    ...
    let loaded = crate::metadata::dwarf::load_dwarf(macho)?
