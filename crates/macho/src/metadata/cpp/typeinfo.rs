...
use crate::metadata::cpp::Result;
use crate::metadata::cpp::model::addr::Va;
use crate::metadata::cpp::model::macho_file::MachoFile;
use crate::metadata::cpp::model::symbol::SymbolTable;
use crate::metadata::dyld::resolve::fixups::{collect_resolved_targets, resolve_pointer_target};
use crate::metadata::dyld::resolve::{ResolutionContext, ResolvedTarget};
...
where
    S: AsRef<[u8]> + ?Sized,
{
    let macho = crate::metadata::cpp::parse_source(source)?;
...
fn typeinfo_class_name(name: &str) -> String {
    crate::metadata::demangle::demangle_symbol(name)
...
impl<'a> PointerResolver<'a> {
    ...
    fn resolve_pointer(&self, va: Va) -> Result<ResolvedTarget> {
        Ok(resolve_pointer_target(&self.ctx, &self.fixups, va)?)
    }

    fn read_u32(&self, va: Va) -> Result<u32> {
        let bytes = self.ctx.macho().read_bytes_at_va(va, 4)?;
        let arr: [u8; 4] = bytes.try_into().map_err(|_| {
            crate::metadata::cpp::error::Error::format(
                "read_bytes_at_va returned unexpected length (want 4)",
            )
        })?;
        Ok(self.ctx.macho().endian().read_u32(arr))
    }
    ...
    fn read_u64(&self, va: Va) -> Result<u64> {
        ...
        let arr: [u8; 8] = bytes.try_into().map_err(|_| {
            crate::metadata::cpp::error::Error::format(
                "read_bytes_at_va returned unexpected length (want 8)",
            )
        })?;
        Ok(self.ctx.macho().endian().read_u64(arr))
    }
}
