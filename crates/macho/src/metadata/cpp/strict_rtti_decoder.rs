...
#[path = "strict_rtti_fixups.rs"]
mod fixups;
use fixups::{PointerFixup, build_pointer_fixups};
...
pub(crate) struct StrictDecoder<'a, 'data> {
    ...
    symbols_by_name: BTreeMap<String, u64>,
...
impl<'a, 'data> StrictDecoder<'a, 'data> {
    ...
    fn type_name(&mut self, symbol: &str, pointer: &StrictPointerObservation) -> Result<String> {
        let va = match &pointer.target {
            StrictPointerTarget::Local { va } => *va,
            StrictPointerTarget::External {
                ...
                .copied()
                .ok_or_else(|| {
                    Error::address("strict RTTI type-name pointer has no local symbol authority")
                })?,
            StrictPointerTarget::Null => {
...
pub fn decode_strict_rtti(
    ...
    decoder.symbols_by_va = symtab
        ...
        .collect();
    decoder.symbols_by_name = symtab
        .symbols()
        .iter()
        .filter(|symbol| symbol.is_defined() && symbol.value != 0)
        .map(|symbol| (symbol.name.to_owned(), symbol.value))
        .collect();
        ...
        match decoder.decode(symbol) {
            ...
            Err(error) => {
                let code = match error.kind {
                    crate::metadata::cpp::CppErrorKind::Unsupported => {
                        StrictRttiGapCode::FamilyUnsupported
                    }
                    crate::metadata::cpp::CppErrorKind::InvalidAddress => {
                        StrictRttiGapCode::PointerUnresolved
                    }
                    crate::metadata::cpp::CppErrorKind::OutOfBounds
                    | crate::metadata::cpp::CppErrorKind::InvalidFormat => {
