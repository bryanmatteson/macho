...
//! façade: build a [`crate::metadata::swift::SwiftTypeIndex`] from a
//! [`crate::core::MachoFile`] or a
//! borrowed byte source.

pub use crate::core::{ext, model};
...
impl<'data> MachoExt<'data> for SwiftTypeIndex {
    ...
    fn parse<'mf>(macho: &'mf MachoFile<'data>) -> crate::metadata::swift::Result<Self>
    where
        'data: 'mf,
    {
        Ok(Self::build(macho))
...
impl SwiftTypeIndex {
    ...
    /// callers select an architecture explicitly through [`crate::core::parse`].
    ...
    pub fn build_with_demangler(
        ...
        let fixups = crate::metadata::dyld::resolve::fixups::collect_resolved_targets(macho);
        ...
        for swift_type in context_descriptors::discover(macho, demangler, &fixups) {
        ...
        let conformances = context_descriptors::discover_conformances(macho, demangler, &fixups);
        let associated_types =
            context_descriptors::discover_associated_types(macho, demangler, &fixups);
        Ok(SwiftTypeIndex {
            types,
            parents,
            conformances,
    ...
    S: AsRef<[u8]> + ?Sized,
{
    match crate::core::parse(source.as_ref())? {
        ...
        crate::core::model::container::MachoContainer::Fat(_) => Err(SwiftError::unsupported(
