...
fn read_pointer_at(macho: &MachoFile<'_>, address: u64) -> Option<u64> {
    match PointerResolver::new(macho)
        ...
        .target
    {
        PointerTarget::Address(address) => Some(address.0),
        PointerTarget::Null | PointerTarget::Import { .. } => None,
    }
}
...
fn validate_field_descriptor(
    ...
    let record_size = match macho.endian() {
        crate::core::format::io::Endian::Little => u16::from_le_bytes([header[10], header[11]]),
        crate::core::format::io::Endian::Big => u16::from_be_bytes([header[10], header[11]]),
    } as u64;
    let count = macho
        .endian()
...
pub(super) fn decode_evidence_witness_table_pattern(
    ...
    use crate::metadata::swift::strict::{
...
fn evidence_witness_provenance(
    value: crate::metadata::swift::strict::StrictWitnessPointerProvenance,
) -> MachoSwiftWitnessPointerProvenanceV1 {
    use crate::metadata::swift::strict::StrictWitnessPointerProvenance;
