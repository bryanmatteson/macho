...
fn validate_conformance_protocol_reference(
    ...
    let protocol_field = add_signed(descriptor_address, (tagged & !1) as i32).ok_or_else(|| {
        gap(
            "swift_metadata_malformed",
            section,
            Some(index),
            "Swift conformance protocol reference overflows",
        )
    })?;
    let protocol = if tagged & 1 == 0 {
        protocol_field
    } else {
        match PointerResolver::new(macho)
            .and_then(|resolver| resolver.observe_at_va(Va(protocol_field)))
            .map(|observation| observation.target)
        {
            Ok(PointerTarget::Address(address)) => address.0,
            Ok(PointerTarget::Import { name, .. }) if !name.is_empty() => {
                // An indirectable protocol reference may bind to a protocol
                // descriptor owned by another image. The bind itself is the
                // complete static authority; the local image has no descriptor
                // bytes to validate here.
                return Ok(());
            }
            _ => {
                return Err(gap(
