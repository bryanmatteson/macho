...
use crate::metadata::dyld::resolve::ResolvedTarget;
use crate::metadata::swift::SwiftDemangler;
use crate::metadata::swift::model::addr::Va;
use crate::metadata::swift::model::macho_file::MachoFile;
use crate::metadata::swift::types::{
...
pub(crate) fn discover(
    ...
    let mut types = discover_section(macho, "__swift5_types", None, demangler, fixups);
...
pub(crate) fn discover_conformances(
    macho: &MachoFile<'_>,
    demangler: &dyn SwiftDemangler,
    fixups: &BTreeMap<u64, ResolvedTarget>,
) -> Vec<SwiftConformanceInfo> {
    ...
    bytes
        ...
        .filter_map(|(index, chunk)| {
            ...
            let (protocol_address, protocol) = read_i32(macho, protocol_field)
                .and_then(|value| {
                    resolve_protocol_reference(macho, protocol_field, value, demangler, fixups)
                })
                .unwrap_or((None, None));
...
pub(crate) fn discover_associated_types(
    ...
    while cursor.0.checked_add(16).is_some_and(|value| value <= end) {
        ...
        let resolved_conforming_type = conforming_type_address.and_then(|address| {
            resolve_mangled_nominal_identity(macho, address, demangler, fixups)
        });
...
fn discover_section(
    ...
    bytes
        ...
        .filter_map(|(index, chunk)| {
            ...
            parse_type_descriptor(macho, descriptor, expected_kind, demangler, fixups)
        })
        .collect()
}
...
fn parse_type_descriptor(
    ...
    Some(SwiftType {
        ...
        fields: kind_has_nominal_field_descriptor(kind)
            .then(|| parse_fields(macho, descriptor, demangler, fixups))
            .flatten(),
        superclass: (kind == SwiftTypeKind::Class)
            .then(|| parse_superclass(macho, descriptor, demangler, fixups))
            .flatten(),
    })
}
...
fn parse_superclass(
    ...
    resolve_mangled_nominal(macho, reference, demangler, fixups)
}
...
fn parse_fields(
    ...
    for index in 0..count {
        ...
        let type_name = read_i32(macho, type_field)
            .filter(|relative| *relative != 0)
            .and_then(|relative| add_signed(type_field, relative))
            .and_then(|address| resolve_mangled_nominal(macho, address, demangler, fixups));
...
fn resolve_mangled_nominal(
    ...
    resolve_mangled_nominal_identity(macho, address, demangler, fixups).map(|(name, _)| name)
}
...
fn resolve_mangled_nominal_identity(
    ...
    if matches!(first, 0x01 | 0x02) {
        ...
        let target = if first == 0x01 {
            ResolvedTarget::Address(direct)
        } else {
            resolve_pointer_target(macho, direct, fixups)?
        };
        return match target {
            ResolvedTarget::Address(descriptor) => {
                context_path(macho, descriptor, 0).map(|path| (path.join("."), Some(descriptor.0)))
            }
            ResolvedTarget::Import { name, .. } => {
                imported_nominal_name(&name, &read_mangled_bytes(macho, address)?, demangler)
                    .map(|name| (name, None))
            }
        };
    }
...
fn resolve_protocol_reference(
    macho: &MachoFile<'_>,
    field: Va,
    relative: i32,
    demangler: &dyn SwiftDemangler,
    fixups: &BTreeMap<u64, ResolvedTarget>,
) -> Option<(Option<Va>, Option<String>)> {
    if relative == 0 {
        return None;
    }
    let indirect = relative & 1 != 0;
    let target = add_signed(field, relative & !1)?;
    let resolved = if indirect {
        resolve_pointer_target(macho, target, fixups)?
    } else {
        ResolvedTarget::Address(target)
    };
    Some(match resolved {
        ResolvedTarget::Address(address) => (
            Some(address),
            context_path(macho, address, 0).map(|path| path.join(".")),
        ),
        ResolvedTarget::Import { name, .. } => (None, imported_protocol_name(&name, demangler)),
    })
}
...
fn resolve_pointer_target(
    ...
    let offset = macho.address_map().va_to_thin_offset(pointer).ok()?.0;
    fixups
        .get(&offset)
        .cloned()
        .or_else(|| read_pointer(macho, pointer).map(ResolvedTarget::Address))
}

fn imported_protocol_name(name: &str, demangler: &dyn SwiftDemangler) -> Option<String> {
    demangler.demangle(name).ok().flatten().and_then(|value| {
        value
            .strip_prefix("protocol descriptor for ")
            .map(str::to_owned)
    })
}

fn imported_nominal_name(
    import: &str,
    symbolic: &[u8],
    demangler: &dyn SwiftDemangler,
) -> Option<String> {
    let suffix = std::str::from_utf8(symbolic.get(5..)?).ok()?;
    let descriptor = import.strip_prefix('_').unwrap_or(import);
    if let Some(base) = descriptor.strip_suffix("Mn") {
        let composed = format!("{base}{suffix}");
        if let Some(value) = demangler.demangle(&composed).ok().flatten() {
            return Some(value);
        }
    }
    demangler.demangle(import).ok().flatten().and_then(|value| {
        value
            .strip_prefix("nominal type descriptor for ")
            .map(str::to_owned)
    })
}
...
mod tests {
    use super::{
        Va, add_signed, imported_nominal_name, imported_protocol_name,
        kind_has_nominal_field_descriptor, swift_type_kind,
    };
    use crate::metadata::swift::PureSwiftDemangler;
    use crate::metadata::swift::types::SwiftTypeKind;
    ...
    #[test]
    fn imported_descriptor_symbols_recover_protocol_and_symbolic_type_names() {
        let demangler = PureSwiftDemangler;
        assert_eq!(
            imported_protocol_name("_$sSQMp", &demangler).as_deref(),
            Some("Swift.Equatable")
        );
        assert_eq!(
            imported_nominal_name(
                "_$s10Foundation4DateVMn",
                &[0x02, 0, 0, 0, 0, b'S', b'g'],
                &demangler,
            )
            .as_deref(),
            Some("Swift.Optional<Foundation.Date>")
        );
    }
