...
use crate::metadata::codesign::superblob::{read_be_u32, read_be_u64};
use crate::metadata::codesign::types::{CSMAGIC_CODEDIRECTORY, CodeDirectory, HashType};

/// Parse a CodeDirectory blob.
pub fn parse_code_directory<'data>(data: &'data [u8]) -> Result<CodeDirectory<'data>> {
    if data.len() < 44 {
        return Err(Error::format("CodeDirectory blob too small"));
    }
