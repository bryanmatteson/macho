        ...
        match lc.kind() {
            LoadCommand::DyldInfo(d) | LoadCommand::DyldInfoOnly(d) => {
                let bind = if d.bind_size > 0 {
                    macho.read_bytes_at(
                        crate::metadata::dyld::model::addr::ThinFileOffset(d.bind_off as u64),
                ...
                let weak = if d.weak_bind_size > 0 {
                    macho.read_bytes_at(
                        crate::metadata::dyld::model::addr::ThinFileOffset(d.weak_bind_off as u64),
                ...
                let lazy = if d.lazy_bind_size > 0 {
                    macho.read_bytes_at(
                        crate::metadata::dyld::model::addr::ThinFileOffset(d.lazy_bind_off as u64),
...
fn interpret_bind_opcodes<'data>(
    ...
    loop {
        ...
        if entries.len() > 10_000_000 {
            return Err(Error::format("bind entry count exceeds limit"));
        }
    }
