...
fn find_rebase_data<'data>(macho: &MachoFile<'data>) -> Result<&'data [u8]> {
    for lc in macho.load_commands() {
        match lc.kind() {
            LoadCommand::DyldInfo(d) | LoadCommand::DyldInfoOnly(d) if d.rebase_size > 0 => {
                return Ok(macho.read_bytes_at(
                    crate::metadata::dyld::model::addr::ThinFileOffset(d.rebase_off as u64),
...
fn interpret_rebase_opcodes(data: &[u8], macho: &MachoFile<'_>) -> Result<Vec<RebaseEntry>> {
    ...
    loop {
        ...
        if entries.len() > 10_000_000 {
            return Err(Error::format("rebase entry count exceeds limit"));
        }
    }
