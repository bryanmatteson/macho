...
use crate::metadata::dyld::error::Result;
use crate::metadata::dyld::format::constants::*;
use crate::metadata::dyld::model::load_command::LoadCommand;
    ...
    if let Some(lc) = macho.find_load_command(|lc| matches!(lc, LoadCommand::DyldExportsTrie(_))) {
        if let Some(ld) = lc.kind().as_linkedit_data() {
            return Ok(macho.read_bytes_at(
                crate::metadata::dyld::model::addr::ThinFileOffset(ld.data_offset as u64),
    ...
    for lc in macho.load_commands() {
        match lc.kind() {
            LoadCommand::DyldInfo(d) | LoadCommand::DyldInfoOnly(d) if d.export_size > 0 => {
                return Ok(macho.read_bytes_at(
                    crate::metadata::dyld::model::addr::ThinFileOffset(d.export_off as u64),
...
fn walk_trie_node<State>(
    ...
    if offset >= data.len() {
        return Err(crate::metadata::dyld::error::Error::format(format!(
            "export trie node offset {offset:#x} is out of bounds"
        )));
    }
    if depth > MAX_TRIE_DEPTH {
        return Err(crate::metadata::dyld::error::Error::format(
    ...
    *node_count = node_count.checked_add(1).ok_or_else(|| {
        crate::metadata::dyld::error::Error::format("export trie node count overflow")
    })?;
    ...
    let terminal_size = usize::try_from(reader.read_uleb128()?).map_err(|_| {
        crate::metadata::dyld::error::Error::format("export terminal size exceeds usize")
    })?;
    let term_start = reader.pos();
    let term_end = term_start.checked_add(terminal_size).ok_or_else(|| {
        crate::metadata::dyld::error::Error::format("export terminal range overflow")
    })?;
    ...
    if terminal_size > 0 {
        ...
        let flags = u32::try_from(terminal_reader.read_uleb128()?)
            .map_err(|_| crate::metadata::dyld::error::Error::format("export flags exceed u32"))?;
    ...
    for _ in 0..edge_count {
        ...
        let child_offset = usize::try_from(reader.read_uleb128()?).map_err(|_| {
            crate::metadata::dyld::error::Error::format("export child offset exceeds usize")
        })?;

        let prev_len = prefix.len();
        let next_len = prev_len.checked_add(label.len()).ok_or_else(|| {
            crate::metadata::dyld::error::Error::format("export name length overflow")
        })?;
...
fn lookup_trie(data: &[u8], name: &str) -> Result<Option<Export>> {
    ...
    loop {
        ...
        if name_pos == name_bytes.len() && terminal_size > 0 {
            ...
            let mut terminal_reader = LebReader::new(&data[term_start..term_end]);
            let flags = u32::try_from(terminal_reader.read_uleb128()?).map_err(|_| {
                crate::metadata::dyld::error::Error::format("export flags exceed u32")
            })?;
            let kind = decode_export_kind(&mut terminal_reader, flags)?;
            if !terminal_reader.is_empty() {
                return Err(crate::metadata::dyld::error::Error::format(
...
mod tests {
    ...
    fn parse_fixture(bytes: &[u8]) -> MachoFile<'_> {
        crate::metadata::dyld::format::macho::parse_macho_file(bytes).unwrap()
    }
    ...
    fn visitor_error_is_propagated_immediately() {
        ...
        let error = visit_exports(&macho, |_export| {
            visits += 1;
            Err(crate::metadata::dyld::error::DyldError::format(
                "visitor stopped",
            ))
        })
        .unwrap_err();
