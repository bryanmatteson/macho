use super::*;
use crate::completeness::completeness_for;
use crate::materialize::materialize_cache_image;
...
impl<'data> DyldCacheFamily<'data> {
    ...
    pub fn reconstruct_image(
        ...
        let container = macho_core::format::parse(&materialized.bytes).map_err(Error::from)?;
        let macho = container
            .first_macho()
