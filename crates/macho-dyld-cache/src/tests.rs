use super::*;
use crate::parse::{IMAGE_INFO_SIZE, IMAGE_TEXT_INFO_SIZE, MAPPING_INFO_SIZE};
...
fn split_family_reconstructs_compact_parseable_image() {
    ...
    let container = macho_core::format::parse(result.bytes()).expect("strict core reparse");
    let macho = container.first_macho().expect("thin image");
    let symbols = macho_core::format::parse_symbol_table(macho).expect("rebuilt symbols");
    assert!(symbols.find_by_name("_symbol").is_some());
    let exports = macho_dyld::parse_exports(macho).expect("retained exports");
...
fn main_entry_file_coordinate_is_rewritten_to_same_va() {
    ...
    let container = macho_core::format::parse(result.bytes()).expect("parse executable");
    ...
    let va = macho
        .address_map()
        .thin_offset_to_va(macho_core::model::addr::ThinFileOffset(main.entry_offset))
        .expect("entry VA");
