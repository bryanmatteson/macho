...
use macho::analysis::reconstruct::cpp::{build_headers_for_mach, build_image_index, unify_images};
use macho_cli::adapters::validate_cpp_header;
...
#[test]
#[ignore = "real clang++ recovery smoke test"]
...
#[test]
#[ignore = "real clang++ header-validation smoke test"]
fn cpp_header_renders_and_validates() {
    ...
    validate_cpp_header(Path::new(&header_path)).expect("clang++ validates emitted header");
}
