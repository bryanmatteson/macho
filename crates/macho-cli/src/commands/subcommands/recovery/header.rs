use macho::analysis::report::{Presence, RecoveryLanguage};
use macho::header_syntax as syntax;

use super::{EntityKind, entity_address, entity_kind, entity_name, entity_presence};
pub(super) fn project_entity(
    entity: &macho::analysis::report::RecoveredEntity,
    language: RecoveryLanguage,
) -> Option<(macho::analysis::report::HeaderDecl, syntax::Decl)> {
    ...
    let name = match language {
        RecoveryLanguage::CAbi => {
            macho::analysis::report::Identifier::new(entity_name(entity)).ok()?
        }
        RecoveryLanguage::Cpp => {
            ...
            let record = macho::analysis::reconstruct::cpp::symbol::parse_symbol(
                raw,
                entity_address(entity),
                None,
            )?;
            let macho::analysis::reconstruct::cpp::CppSymbolKind::Function { decl } = record.kind
            else {
                return None;
            };
            ...
            macho::analysis::report::Identifier::new(decl.name.leaf()?.to_owned()).ok()?
        }
    };
    let return_type = match &entity.signature.return_type {
        Fact::Known {
            ...
            strength,
            ..
        } if *strength != macho::analysis::report::EvidenceStrength::Inferred => ty.clone(),
        _ => return None,
    };
    let recovered_parameters = match &entity.signature.parameters {
        Fact::Known {
            ...
            strength,
            ..
        } if *strength != macho::analysis::report::EvidenceStrength::Inferred => value,
        _ => return None,
    };
    ...
    for (index, parameter) in recovered_parameters.iter().enumerate() {
        ...
        let name = macho::analysis::report::Identifier::new(raw_name)
            .or_else(|_| macho::analysis::report::Identifier::new(format!("arg{index}")))
            .ok()?;
    ...
    let calling_convention = match &entity.signature.calling_convention {
        Fact::Known {
            value, strength, ..
        } if *strength != macho::analysis::report::EvidenceStrength::Inferred
...
fn project_variable(
    ...
    let name = macho::analysis::report::Identifier::new(entity_name(entity)).ok()?;
...
fn project_type(
    ...
    use macho::analysis::report::{Fact, HeaderDecl, LayoutCompleteness, RecordKind};
    ...
    let path = entity_name(entity)
        .split("::")
        .map(|component| macho::analysis::report::Identifier::new(component.to_owned()).ok())
        .collect::<Option<Vec<_>>>()?;
    if path.len() != 1 {
        return None;
    }
    let wire_path = macho::analysis::report::NonEmpty::new(path).ok()?;
...
fn wire_field(
    field: &macho::analysis::report::RecoveredField,
) -> Option<macho::analysis::report::HeaderField> {
    use macho::analysis::report::{Access, EvidenceStrength, Fact, TypeEvidence};
    let name = match &field.name {
        Fact::Known { value, .. } => {
            macho::analysis::report::Identifier::new(value.clone()).ok()?
        }
        _ => return None,
    };
    ...
    Some(macho::analysis::report::HeaderField {
        name,
        ty,
        offset,
        bit_width,
...
fn syntax_field(field: &macho::analysis::report::HeaderField) -> Option<syntax::Field> {
    Some(syntax::Field {
        ...
        access: match field.access {
            macho::analysis::report::Access::Public => syntax::Access::Public,
            macho::analysis::report::Access::Protected => syntax::Access::Protected,
            macho::analysis::report::Access::Private => syntax::Access::Private,
            macho::analysis::report::Access::Unspecified => syntax::Access::Unspecified,
        },
    })
}

fn syntax_type(value: &macho::analysis::report::HeaderType) -> Option<syntax::Type> {
    use macho::analysis::report as wire;
    Some(match value {
