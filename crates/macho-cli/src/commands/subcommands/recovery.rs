...
fn project_headers(report: &mut RecoveryReport, language: RecoveryLanguage) -> Result<()> {
    for slice in report.slices.as_mut_slice() {
        ...
        let unit = TreeSitterHeaderParser
            ...
            .map_err(anyhow::Error::new)?;
        let syntax_report = macho::header_syntax::validate(&unit, ValidationLimits::default())
            .map_err(anyhow::Error::new)?;
...
fn load_header_roots(
    values: &[String],
    language: RecoveryLanguage,
    limits: macho::analysis::report::RecoveryLimits,
    ...
    for value in values {
        ...
        roots.push(HashedHeaderRoot {
            logical_label,
            content_hash: macho::analysis::report::ContentHash::new(sha256(&root_bytes))
    ...
    for (relative, path) in paths {
        ...
        files.push(LoadedHeaderFile {
            hashed: HashedHeaderFile {
                ...
                content_sha256: macho::analysis::report::ContentHash::new(sha256(&bytes))
                    .expect("SHA-256 is a valid content hash"),
...
fn entity_presence(entity: &macho::analysis::report::RecoveredEntity) -> Presence {
...
fn entity_kind(entity: &macho::analysis::report::RecoveredEntity) -> Option<EntityKind> {
    match &entity.role {
        Fact::Known { value, .. } => Some(match value {
            macho::analysis::report::EntityRole::Function => EntityKind::Function,
            macho::analysis::report::EntityRole::Data
            | macho::analysis::report::EntityRole::CppStaticData => EntityKind::Data,
            macho::analysis::report::EntityRole::Tls => EntityKind::Tls,
            macho::analysis::report::EntityRole::RuntimeArtifact => EntityKind::RuntimeArtifact,
            macho::analysis::report::EntityRole::CppMethod => EntityKind::Method,
            macho::analysis::report::EntityRole::Type => EntityKind::Type,
            macho::analysis::report::EntityRole::Typeinfo => EntityKind::Typeinfo,
            macho::analysis::report::EntityRole::Vtable
            | macho::analysis::report::EntityRole::Vtt => EntityKind::Vtable,
            macho::analysis::report::EntityRole::Thunk => EntityKind::Thunk,
            macho::analysis::report::EntityRole::Guard => EntityKind::Guard,
            macho::analysis::report::EntityRole::Unknown => EntityKind::Unknown,
        }),
        _ => None,
    }
}

fn entity_names(entity: &macho::analysis::report::RecoveredEntity) -> Vec<&str> {
...
fn entity_name(entity: &macho::analysis::report::RecoveredEntity) -> String {
    entity_names(entity)
        .first()
        .copied()
        .unwrap_or("<unknown>")
...
fn entity_address(entity: &macho::analysis::report::RecoveredEntity) -> Option<u64> {
...
fn role_name(entity: &macho::analysis::report::RecoveredEntity) -> &'static str {
