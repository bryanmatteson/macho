...
use anyhow::Result;
use macho::format::constants::{CPU_TYPE_ARM64, CPU_TYPE_X86, CPU_TYPE_X86_64, VmProtection};
use macho::patch::{
...
fn validate_instruction_window(
    ...
    let insn_arch = match arch {
        PatchArch::X86_64 => macho::insn::Arch::X86_64,
        PatchArch::Arm64 => macho::insn::Arch::Arm64,
        PatchArch::Arm64e => macho::insn::Arch::Arm64e,
    ...
    while offset < window.len() {
        ...
        let instruction = macho::insn::decode_one(&window[offset..], va, insn_arch).map_err(
            |error| {
                anyhow::anyhow!(
