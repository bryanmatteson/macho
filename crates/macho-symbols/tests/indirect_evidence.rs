...
fn malformed_stride_table_symbol_and_utf8_reject_without_partial_results() {
    ...
    assert!(
        decode_indirect_bindings(&macho_core::format::parse_macho_file(&stride).unwrap(), 8)
            .is_err()
    );
    ...
    assert!(
        decode_indirect_bindings(
            &macho_core::format::parse_macho_file(&absent_symbol).unwrap(),
            8
        )
        .is_err()
    );
    ...
    utf8[0x271] = 0xff;
    assert!(
        decode_indirect_bindings(&macho_core::format::parse_macho_file(&utf8).unwrap(), 8).is_err()
    );

    let mut table = fixture(&[0]);
    table[DYSYMTAB + 56..DYSYMTAB + 60].copy_from_slice(&0x2ff_u32.to_le_bytes());
    assert!(
        decode_indirect_bindings(&macho_core::format::parse_macho_file(&table).unwrap(), 8)
            .is_err()
    );
    ...
    assert!(
        decode_indirect_bindings(
            &macho_core::format::parse_macho_file(&host_overflow).unwrap(),
            8
        )
        .is_err()
    );
