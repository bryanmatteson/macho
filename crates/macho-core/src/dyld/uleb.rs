...
impl<'data> LebReader<'data> {
    ...
    pub fn read_u8(&mut self) -> Result<u8> {
        if self.pos >= self.data.len() {
            return Err(Error::Format("unexpected end of data reading byte".into()));
        }
    ...
    pub fn peek_u8(&self) -> Result<u8> {
        if self.pos >= self.data.len() {
            return Err(Error::Format("unexpected end of data peeking byte".into()));
        }
    ...
    pub fn read_uleb128(&mut self) -> Result<u64> {
        ...
        loop {
            if self.pos >= self.data.len() {
                return Err(Error::Format("unexpected end of data in ULEB128".into()));
            }
            ...
            shift += 7;
            if shift > 70 {
                return Err(Error::Format("ULEB128 too large".into()));
            }
        }
    }
    ...
    pub fn read_sleb128(&mut self) -> Result<i64> {
        ...
        loop {
            if self.pos >= self.data.len() {
                return Err(Error::Format("unexpected end of data in SLEB128".into()));
            }
            ...
            if shift > 70 {
                return Err(Error::Format("SLEB128 too large".into()));
            }
        }
        // Sign extend
    ...
    pub fn skip(&mut self, n: usize) -> Result<()> {
        let new = self
            .pos
            .checked_add(n)
            .ok_or_else(|| Error::Format("skip overflow".into()))?;
        if new > self.data.len() {
            return Err(Error::Format("skip past end of data".into()));
        }
        self.pos = new;
        Ok(())
    }
