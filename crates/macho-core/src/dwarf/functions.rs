...
use gimli::{
    ...
    DW_TAG_formal_parameter, DW_TAG_pointer_type, DW_TAG_reference_type,
    DW_TAG_restrict_type, DW_TAG_rvalue_reference_type, DW_TAG_structure_type,
    DW_TAG_subprogram, DW_TAG_subroutine_type, DW_TAG_typedef, DW_TAG_union_type,
    DW_TAG_unspecified_parameters, DW_TAG_volatile_type, EndianSlice, LittleEndian, UnitOffset,
};
...
fn parse_parameter(
    ...
    let (param_name, type_val) = if let Some(origin_offset) =
        entry.attr_value(DW_AT_abstract_origin).and_then(type_ref_offset)
    {
