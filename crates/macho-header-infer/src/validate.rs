...
pub fn validate_response(
    ...
    let mut source = macho_header_syntax::render(&unit)
        .map_err(|error| ArtifactError::Invalid(format!("render typed fragments: {error}")))?;
    let validation = macho_header_syntax::validate(&unit, ValidationLimits::default())
        .map_err(|error| ArtifactError::Invalid(format!("validate typed fragments: {error}")))?;
...
fn validate_operation(
    bundle: &HypothesisBundle,
    hypothesis: &crate::ProposedHypothesis,
...
fn diagnostic(
    ...
    message: String,
    hypothesis_id: &macho_analysis::report::HypothesisId,
