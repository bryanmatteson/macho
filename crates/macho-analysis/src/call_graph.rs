...
//! assembled only from call instructions retained by [`crate::control_flow`].
//! Indirect calls and direct targets without an admitted function identity
//! remain explicit unresolved callsites rather than disappearing from the
//! result.
...
fn node_from_function(
    function: &crate::functions::RecoveredFunction,
    control_flow: &ControlFlowIndex,
) -> DirectCallNode {
...
mod tests {
    use super::*;
    use crate::control_flow::{ControlFlowIndex, ControlFlowLimits};
    ...
    fn retention_and_work_budgets_never_claim_completion() {
        ...
        two_edges_bytes[0x106..0x10a].copy_from_slice(&0x26_i32.to_le_bytes());
