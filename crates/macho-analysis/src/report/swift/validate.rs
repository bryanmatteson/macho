...
fn validate_swift_slice(slice: &SwiftSliceReport) -> crate::Result<()> {
    ...
    for entity in &slice.entities {
        ...
        for gap in &entity.gaps {
            if !gap_ids.insert(gap.id.as_str()) {
                return Err(crate::AnalysisError::validation("duplicate Swift gap ID"));
            }
        }
            ...
            SwiftObservationDisposition::Included {
                ...
                for id in included.as_slice() {
                    let entity = entity_index.get(id.as_str()).ok_or_else(|| {
                        crate::AnalysisError::validation("dangling Swift entity reference")
                    })?;
...
fn validate_swift_ref(value: &SwiftEntityRef, entities: &BTreeSet<&str>) -> crate::Result<()> {
