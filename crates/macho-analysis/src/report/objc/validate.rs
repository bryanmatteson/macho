...
pub(super) fn validate_objc_slice(slice: &ObjCSliceReport) -> crate::Result<()> {
...
fn validate_totals(slice: &ObjCSliceReport) -> crate::Result<()> {
    let count = |presence| {
        slice
            .entities
            .iter()
...
fn validate_executions(slice: &ObjCSliceReport, diagnostics: &BTreeSet<&str>) -> crate::Result<()> {
...
fn validate_refs(values: &[ObjCTypeRef], entities: &BTreeSet<&str>) -> crate::Result<()> {
    for value in values {
...
fn insert_member(id: &ObjCMemberId, members: &mut BTreeSet<String>) -> crate::Result<()> {
...
fn require(values: &BTreeSet<&str>, value: &str, kind: &str) -> crate::Result<()> {
...
fn invalid<T>(message: impl Into<String>) -> crate::Result<T> {
    Err(crate::AnalysisError::validation(message.into()))
}
