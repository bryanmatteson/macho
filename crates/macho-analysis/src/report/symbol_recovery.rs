...
use macho_core::model::SectionType;
use macho_core::model::container::MachoContainer;
use macho_core::{MachoFile, Section, SymbolTable};
...
pub fn recover_symbol_container(
    ...
    report
        .validate()
        .map_err(|error| crate::AnalysisError::validation(error.to_string()))?;
    Ok(report)
}
...
fn request_digest(request: &RecoveryRequestSummary) -> crate::Result<RequestDigest> {
    Ok(RequestDigest::new(digest_bytes(
...
fn build_entity(symbol: &macho_core::Symbol<'_>, input: EntityBuildInput) -> RecoveredEntity {
...
fn classify_role(
    ...
    if classification == LinkageClassification::ItaniumCpp {
        ...
        } else if crate::reconstruct::cpp::symbol::parse_symbol(symbol.name, None, None)
            .is_some_and(|record| {
                matches!(
                    record.kind,
                    crate::reconstruct::cpp::CppSymbolKind::Data { .. }
                )
            })
        {
...
fn classify_linkage(name: &str) -> LinkageClassification {
    ...
    if candidate.starts_with("_Z") {
        if macho_symbols::demangle::demangle_cpp_symbol(name).is_some() {
...
fn normalize_name(name: &str) -> String {
    macho_symbols::demangle::demangle_symbol(name).unwrap_or_else(|| {
        name.strip_prefix('_')
...
fn symbol_presence(symbol: &macho_core::Symbol<'_>) -> Presence {
