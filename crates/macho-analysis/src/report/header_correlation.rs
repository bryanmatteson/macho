...
///
/// Inputs have already been parsed by `macho-header-syntax`; textual identifier
...
fn entity_terminal_name(entity: &RecoveredEntity, language: RecoveryLanguage) -> Option<String> {
    if language == RecoveryLanguage::Cpp
        ...
        && let Some(record) = crate::reconstruct::cpp::symbol::parse_symbol(&value.raw, None, None)
        && let crate::reconstruct::cpp::CppSymbolKind::Function { decl } = record.kind
    {
