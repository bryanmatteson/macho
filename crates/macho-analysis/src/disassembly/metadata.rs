...
use crate::report::disassembly::{DisassemblyIssue, DisassemblyLabel, SymbolSource};
...
impl<'macho> Metadata<'macho> {
    ...
    pub(crate) fn find_file_section(
        &self,
        va: u64,
    ) -> Option<&'macho macho_core::model::section::Section> {
...
pub(crate) fn collect_metadata<'macho>(
    ...
    if !requested.is_empty() {
        if has_symtab {
            ...
            match macho_core::format::fold_symbols(macho, metadata, |state, symbol| {
        ...
        let failure_state = metadata.failure_state();
        match macho_dyld::fold_exports(macho, metadata, |state, export| {
    ...
    if has_objc {
        ...
        let failure_state = metadata.failure_state();
        match macho_objc::fold_method_imps(macho, metadata, |state, method| {
...
fn export_va(macho: &MachoFile<'_>, kind: &ExportKind) -> macho_dyld::Result<Option<u64>> {
    match kind {
        ExportKind::Regular { address } | ExportKind::ThreadLocal { address } => {
            ...
            macho
                ...
                .checked_add(*address)
                .map(Some)
                .ok_or_else(|| macho_dyld::DyldError::address("export virtual address overflows"))
        }
        ExportKind::Absolute { .. }
        | ExportKind::Reexport { .. }
...
fn observation(
    ...
    let display_name = if demangle {
        macho_symbols::demangle::demangle_symbol(raw_name).unwrap_or_else(|| raw_name.to_owned())
    } else {
        raw_name.to_owned()
    };
