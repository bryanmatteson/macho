...
pub(crate) fn decode_slice(
    ...
    let image_hash = if container_kind == ContainerKind::Thin {
        ...
        ContentHash::new(crate::report::sha256_hex(slice.macho.bytes()))
            .expect("SHA-256 is canonical lowercase hexadecimal")
    };
    let uuid = slice
        .macho
        .uuid()
        .map(macho_core::format_uuid)
...
fn decode_region(
    ...
    loop {
        ...
        match decode_at(tail, cursor, slice.arch, &mut work) {
            Ok(instruction) => {
                ...
                work.charge_probe(bytes.len());
                let text =
                    macho_insn::disassemble_one(bytes, cursor, slice.arch).map_err(|error| {
...
fn extend_x86_gap(
    ...
    while length < selected_len {
        ...
        if decode_at(&tail[offset..], va + length, macho_insn::Arch::X86_64, work).is_ok() {
            return ExtendedGap {
                length,
...
fn decode_at(
    tail: &[u8],
    va: u64,
    arch: macho_insn::Arch,
    work: &mut DecodeWork,
) -> Result<Insn, macho_insn::DecodeError> {
    ...
    macho_insn::decode_one(&tail[..window_len], va, arch)
}
