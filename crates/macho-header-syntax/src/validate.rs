...
use crate::{Decl, IdentifierPath, ObjectiveCForwardKind, TranslationUnit, Type};
...
impl Budget {
    ...
    fn objc_members(
        &mut self,
        methods: &[crate::ObjectiveCMethod],
        properties: &[crate::ObjectiveCProperty],
        ...
        match ty {
            ...
            Type::Named {
                ...
                for argument in template_arguments {
                    self.node()?;
                    if let crate::TemplateArgument::Type(ty) = argument {
                        self.ty(ty, depth + 1)?;
                    }
                }
                Ok(())
...
fn validate_type(
    ...
    match ty {
        ...
        Type::Named {
            ...
            let resolved = if matches!(tag, crate::NamedTypeTag::Protocol) {
...
mod tests {
    ...
    fn objc_class_forward_resolves_reparsed_named_pointer_types() {
        let unit = crate::TreeSitterHeaderParser
            .parse(
                Language::ObjectiveC,
