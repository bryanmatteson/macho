...
That header was never in the file. `macho` reconstructed the complete declaration from Objective-C metadata embedded in the binary.
...
Everything speaks `--format text` for humans and `--format json` for pipelines; `audit` also emits **SARIF 2.1** so findings drop straight into GitHub code scanning.

## Highlights

- **Complete header reconstruction.** `objc --headers` and `swift --headers` rebuild complete declarations from compiled metadata. Missing types, attributes, relationships, or signatures are bugs in the extractor.
...
```bash
cargo install macho-lib      # the library façade (crate: macho-lib, imported as `macho`)
cargo install --path crates/macho-cli   # the `macho` CLI, from a checkout
```
...
# Reconstructed headers from stripped binaries
...
Every command accepts `--format text|json` (`audit` also `sarif`) and `--color auto|always|never`. JSON uses a versioned envelope, machine formats never contain ANSI escapes, and errors go only to stderr — so `macho ... --format json | jq` always sees clean output. Semantic roles and the terminal theme come from the pinned Termosaic presentation library; the reusable Mach-O crates stay output-neutral.
...
The workspace is layered so dependencies flow one direction — a byte-safe core at the bottom, metadata leaves and analysis above it, then mutation, semantic workflow, the façade, and the CLI on top. This is enforced, not aspirational:
