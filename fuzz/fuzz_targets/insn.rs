...
fuzz_target!(|data: &[u8]| {
    for base in BASES {
        for arch in [Arch::X86_64, Arch::Arm64, Arch::Arm64e] {
            let strict: Vec<_> = macho::insn::decode_iter(data, base, arch).collect();
            let lossy = macho::insn::decode_lossy(data, base, arch);
            assert_eq!(
                strict,
                macho::insn::decode_iter(data, base, arch).collect::<Vec<_>>()
            );
            assert_eq!(lossy, macho::insn::decode_lossy(data, base, arch));
            let covered = lossy
                .instructions
                .iter()
                .map(|insn| insn.len)
            ...
            let formatted = macho::insn::disassemble(data, base, arch);
